...
void mach_msg_destroy(msg) mach_msg_header_t *msg;
{
...
/**
 * @brief Release a port right referenced from a message.
 *
 * Depending on @p type the appropriate kernel call is made to drop the right.
 */
static void mach_msg_destroy_port(port, type) mach_port_t port;
...
static void
    /**
     * @brief Deallocate out-of-line memory referenced in a message.
     *
     * Wrapper around vm_deallocate that checks for zero @p size.
     */
    mach_msg_destroy_memory(addr, size) vm_offset_t addr;
vm_size_t size;
{
  if (size != 0)
