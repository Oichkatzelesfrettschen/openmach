...
/**
 * @brief General memory allocation with alignment and range constraints.
 *
 * Searches the regions of @p lmm for a block that meets size, alignment and
 * address range requirements.  The block is removed from the free list and its
 * address returned.
 *
 * @param lmm       The LMM arena to allocate from.
 * @param size      Minimum number of bytes requested.
 * @param flags     Allocation constraint flags.
 * @param align_bits Alignment as log₂ of the boundary.
 * @param align_ofs Offset from the alignment boundary.
 * @param in_min    Lowest acceptable address.
 * @param in_size   Size of the acceptable address range.
 *
 * @return Pointer to the allocated memory or NULL on failure.
 */
void *lmm_alloc_gen(lmm_t *lmm, vm_size_t size, unsigned flags, int align_bits,
                    vm_offset_t align_ofs, vm_offset_t in_min,
                    vm_size_t in_size) {
  ...
  for (reg = lmm->regions; reg; reg = reg->next) {
    ...
    if ((flags & ~reg->flags) || ((vm_offset_t)reg >= in_max) ||
    ...
    for (nodep = &reg->nodes; node = *nodep; nodep = &node->next) {
      ...
      /* Now compute the address at which the allocated chunk would have to
       * start.  */
      ...
      /* See if the block at the adjusted address is still entirely within the
       * node.  */
