...
/**
 * @brief Add a memory region to the low memory manager.
 *
 * This function inserts a new region into the region list sorted by
 * priority and size.
 *
 * @param lmm   Low memory manager instance to add the region to.
 * @param min   Starting address of the region.
 * @param size  Size of the region in bytes.
 * @param flags Attributes associated with the region.
 * @param pri   Priority of the region.
 *
 * @return None.
 */
void lmm_add(lmm_t *lmm, vm_offset_t min, vm_size_t size, lmm_flags_t flags,
             lmm_pri_t pri) {
  ...
  if ((max < min) ||
      (max - min < sizeof(struct lmm_region) + sizeof(struct lmm_node)))
    return;
  ...
  for (rp = &lmm->regions;
       (r = *rp) &&
       ((r->pri > pri) || ((r->pri == pri) && (r->size > reg->size)));
       rp = &r->next)
    ;
  reg->next = r;
  *rp = reg;
