...
/**
 * @brief Locate the next free memory chunk at or above a given address.
 *
 * On return, @p *inout_addr contains the address of the next free block,
 * @p *out_size its size and @p *out_flags the region flags of that block.
 *
 * @param lmm        Arena to search.
 * @param inout_addr In: starting address hint; Out: location of free block.
 * @param out_size   Out: size of the found block.
 * @param out_flags  Out: region flags for the block.
 */
void lmm_find_free(lmm_t *lmm, vm_offset_t *inout_addr, vm_size_t *out_size,
  ...
  for (reg = lmm->regions; reg; reg = reg->next) {
    ...
    if ((reg->nodes == 0) || ((vm_offset_t)reg + reg->size <= start_addr) ||
    ...
    for (node = reg->nodes; node; node = node->next) {
      ...
      if ((vm_offset_t)node + node->size > start_addr) {
        if ((vm_offset_t)node > start_addr) {
          ...
          lowest_size = node->size - (lowest_addr - (vm_offset_t)node);
        }
