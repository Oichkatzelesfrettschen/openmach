...
/**
 * @brief Send a message, retrying if interrupted.
 */
...
/**
 * @brief Receive a message, retrying if interrupted.
 */
msg_return_t msg_receive(msg_header_t *header, msg_option_t option,
                         msg_timeout_t timeout) {
  register msg_return_t result;
...
/**
 * @brief Send a message and wait for a reply, handling interrupts.
 */
msg_return_t msg_rpc(msg_header_t *header, msg_option_t option,
                     msg_size_t rcv_size, msg_timeout_t send_timeout,
                     msg_timeout_t rcv_timeout) {
  register msg_return_t result;

  result = msg_rpc_trap(header, option, header->msg_size, rcv_size,
  ...
  if ((result == RCV_INTERRUPTED) && !(option & RCV_INTERRUPT))
    do
      result = msg_receive_trap(header, option, rcv_size,
                                header->msg_local_port, rcv_timeout);
    while (result == RCV_INTERRUPTED);
