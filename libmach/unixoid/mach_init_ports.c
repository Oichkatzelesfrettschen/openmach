...
/**
 * @brief Populate standard task ports from the kernel.
 */
void mach_init_ports(void) {
  ...
  kr = mach_ports_lookup(mach_task_self(), &ports, &ports_count);
  if ((kr != KERN_SUCCESS) || (ports_count < MACH_PORTS_SLOTS_USED))
    return;
  ...
  (void)vm_deallocate(mach_task_self(), (vm_offset_t)ports,
