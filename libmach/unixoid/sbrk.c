...
/**
 * @brief Grow the data segment by @p size bytes.
 */
vm_offset_t sbrk(int size) {
  int size;
  {
    vm_offset_t addr;
    kern_return_t ret;
  ...
  vm_offset_t _brk(addr) vm_offset_t addr;
  {
    ...
    if (rnd_addr < rnd_brk)
      /**
       * @brief Set the break address directly.
       */
      vm_offset_t _brk(vm_offset_t addr) {
        else if (rnd_addr > rnd_brk) ret =
            vm_allocate(mach_task_self(), &rnd_brk, rnd_addr - rnd_brk, FALSE);
        if (ret != KERN_SUCCESS) {
#if true_unix
          errno = ENOMEM;
#endif
    ...
    vm_offset_t brk(addr) vm_offset_t addr;
    {
      /**
       * @brief Public interface to set the program break.
       */
      vm_offset_t brk(vm_offset_t addr) {
        addr = minbrk;
