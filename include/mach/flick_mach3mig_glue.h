 ...
 * This file is included from every stub source code (.c) file generated by that
 * backend. Stubs are built primarily out of invocations of these macros.
 */
...
struct flick_mach3mig_rpc_desc {
  ...
  /* Before calling flick_mach3mig_rpc(),
     the client stub sets this to the offset of the end of the data it
     marshaled. It always starts marshaling just after the Mach message header.
   */
...
#define flick_alloc_mach_vm(size)                                              \
  ({                                                                           \
    vm_address_t addr;                                                         \
    if (_err = vm_allocate(mach_task_self(), &addr, (size), 1))                \
      return _err;                                                             \
    (void *)addr;                                                              \
  })
#define flick_free_mach_vm(addr, size)                                         \
  if (_err = vm_deallocate(mach_task_self(), (addr), (size)))                  \
    return _err;
...
#define flick_mach3mig_encode_new_glob(max_size)                               \
  {                                                                            \
    while (_desc.d.send_end_ofs + (max_size) > _desc.d.msg_buf_size)           \
      if (_err = flick_mach3mig_rpc_grow_buf(&_desc))                          \
        return _err;                                                           \
...
#define flick_mach3mig_encode_char16(_ofs, _data)                              \
  flick_mach3mig_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 8,         \
                             signed16_t);
#define flick_mach3mig_encode_signed8(_ofs, _data)                             \
  flick_mach3mig_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,          \
                             signed8_t);
#define flick_mach3mig_encode_unsigned8(_ofs, _data)                           \
  flick_mach3mig_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,          \
                             unsigned8_t);
#define flick_mach3mig_encode_signed16(_ofs, _data)                            \
  flick_mach3mig_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,        \
                             signed16_t);
#define flick_mach3mig_encode_unsigned16(_ofs, _data)                          \
  flick_mach3mig_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,        \
                             unsigned16_t);
#define flick_mach3mig_encode_signed32(_ofs, _data)                            \
  flick_mach3mig_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_32, 32,        \
                             signed32_t);
...
#define flick_mach3mig_encode_port(_ofs, _data, _adjust)                       \
  {                                                                            \
    ...
    flick_mach3mig_encode_prim(_ofs, _data,                                    \
                               _adjust ? MACH_MSG_TYPE_MOVE_SEND               \
                                       : MACH_MSG_TYPE_COPY_SEND,              \
                               32, mach_port_t);                               \
  }

/* Array type descriptors.  */
#define flick_mach3mig_array_encode_type(_ofs, _name, _bits, _ctype, _num,     \
                                         _inl)                                 \
  {                                                                            \
...
#define flick_mach3mig_array_encode_long_type(_ofs, _name, _bits, _ctype,      \
                                              _num, _inl)                      \
  {                                                                            \
...
#define flick_mach3mig_array_encode_boolean_type(_ofs, _num, _inl, _long)      \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_BOOLEAN, 32,          \
                                     signed32_t, _num, _inl);
#define flick_mach3mig_array_encode_char8_type(_ofs, _num, _inl, _long)        \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_CHAR, 8, signed8_t,   \
                                     _num, _inl);
#define flick_mach3mig_array_encode_char16_type(_ofs, _num, _inl, _long)       \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_INTEGER_16, 8,        \
                                     signed16_t, _num, _inl);
#define flick_mach3mig_array_encode_signed8_type(_ofs, _num, _inl, _long)      \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_INTEGER_8, 8,         \
                                     signed8_t, _num, _inl);
#define flick_mach3mig_array_encode_unsigned8_type(_ofs, _num, _inl, _long)    \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_INTEGER_8, 8,         \
                                     unsigned8_t, _num, _inl);
#define flick_mach3mig_array_encode_signed16_type(_ofs, _num, _inl, _long)     \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_INTEGER_16, 16,       \
                                     signed16_t, _num, _inl);
#define flick_mach3mig_array_encode_unsigned16_type(_ofs, _num, _inl, _long)   \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_INTEGER_16, 16,       \
                                     unsigned16_t, _num, _inl);
#define flick_mach3mig_array_encode_signed32_type(_ofs, _num, _inl, _long)     \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_INTEGER_32, 32,       \
                                     signed32_t, _num, _inl);
#define flick_mach3mig_array_encode_unsigned32_type(_ofs, _num, _inl, _long)   \
  flick_mach3mig_array_encode##_long(_ofs, MACH_MSG_TYPE_INTEGER_32, 32,       \
                                     unsigned32_t, _num, _inl);

/* Array elements.  */
#define flick_mach3mig_array_encode_new_glob(max_size)                         \
  flick_mach3mig_encode_new_glob(max_size)
...
#define flick_mach3mig_array_encode_boolean(_ofs, _data)                       \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_BOOLEAN, 32,     \
                                   signed32_t);
#define flick_mach3mig_array_encode_char8(_ofs, _data)                         \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_CHAR, 8,         \
                                   signed8_t);
#define flick_mach3mig_array_encode_char16(_ofs, _data)                        \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 8,   \
                                   signed16_t);
#define flick_mach3mig_array_encode_signed8(_ofs, _data)                       \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,    \
                                   signed8_t);
#define flick_mach3mig_array_encode_unsigned8(_ofs, _data)                     \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,    \
                                   unsigned8_t);
#define flick_mach3mig_array_encode_signed16(_ofs, _data)                      \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,  \
                                   signed16_t);
#define flick_mach3mig_array_encode_unsigned16(_ofs, _data)                    \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,  \
                                   unsigned16_t);
#define flick_mach3mig_array_encode_signed32(_ofs, _data)                      \
  flick_mach3mig_array_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_32, 32,  \
                                   signed32_t);
...
#define flick_mach3mig_array_encode_ool_start(_ofs, _size)                     \
  {                                                                            \
    ...
    struct {                                                                   \
      struct {                                                                 \
        void *msg_buf;                                                         \
        vm_size_t msg_buf_size, send_end_ofs;                                  \
      } d;                                                                     \
    } _desc;                                                                   \
    void *_e_chunk;                                                            \
                                                                               \
    ...
    if (_err = vm_allocate(mach_task_self(), _p, _desc.d.msg_buf_size, 1))     \
      return _err;                                                             \
    _desc.d.msg_buf = (void *)*_p;                                             \
    _desc.d.send_end_ofs = 0;
...
#define flick_mach3mig_decode_new_chunk(size)                                  \
  {                                                                            \
    flick_iftypecheck(if (_desc.d.rcv_ofs + (size) >                           \
                          _desc.d.rcv_end_ofs) return MIG_TYPE_ERROR;);        \
    _d_chunk = (void *)_desc.d.msg_buf + _desc.d.rcv_ofs;                      \
  }
#define flick_mach3mig_decode_end_chunk(size) _desc.d.rcv_ofs += (size);
...
#define flick_mach3mig_decode_boolean(_ofs, _data)                             \
  flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_BOOLEAN, 32,           \
                             signed32_t);
...
#define flick_mach3mig_decode_char16(_ofs, _data)                              \
  flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 8,         \
                             signed16_t);
#define flick_mach3mig_decode_signed8(_ofs, _data)                             \
  flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,          \
                             signed8_t);
#define flick_mach3mig_decode_unsigned8(_ofs, _data)                           \
  flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,          \
                             unsigned8_t);
#define flick_mach3mig_decode_signed16(_ofs, _data)                            \
  flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,        \
                             signed16_t);
#define flick_mach3mig_decode_unsigned16(_ofs, _data)                          \
  flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,        \
                             unsigned16_t);
#define flick_mach3mig_decode_signed32(_ofs, _data)                            \
  flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_32, 32,        \
                             signed32_t);
...
#define flick_mach3mig_decode_port(_ofs, _data, _adjust)                       \
  {                                                                            \
    flick_mach3mig_decode_prim(_ofs, _data, MACH_MSG_TYPE_PORT_SEND, 32,       \
                               mach_port_t);                                   \
    if (_adjust != 1) {                                                        \
      if (_err = mach_port_mod_refs(mach_task_self(), (_data),                 \
                                    MACH_PORT_RIGHT_SEND, _adjust - 1))        \
...
/* Array type descriptors.  */
#define flick_mach3mig_array_decode_type(_ofs, _name, _bits, _ctype, _num,     \
                                         _inl)                                 \
  {                                                                            \
    mach_msg_type_t *_p = (void *)(_e_chunk + _ofs);                           \
        flick_iftypecheck( ({								\
...
#define flick_mach3mig_array_decode_long_type(_ofs, _name, _bits, _ctype,      \
                                              _num, _inl)                      \
  {                                                                            \
    mach_msg_type_long_t *_p = (void *)(_e_chunk + _ofs);                      \
        flick_iftypecheck( ({								\
...
#define flick_mach3mig_array_decode_boolean_type(_ofs, _num, _inl, _long)      \
  flick_mach3mig_array_decode##_long(_ofs, MACH_MSG_TYPE_BOOLEAN, 32,          \
                                     signed32_t, _num, _inl);
#define flick_mach3mig_array_decode_char8_type(_ofs, _num, _inl, _long)        \
  flick_mach3mig_array_decode##_long(_ofs, MACH_MSG_TYPE_CHAR, 8, signed8_t,   \
                                     _num, _inl);
#define flick_mach3mig_array_decode_char16_type(_ofs, _num, _inl, _long)       \
  flick_mach3mig_array_decode##_long(_ofs, MACH_MSG_TYPE_INTEGER_16, 8,        \
                                     signed16_t, _num, _inl);
#define flick_mach3mig_array_decode_signed8_type(_ofs, _num, _inl, _long)      \
  flick_mach3mig_array_decode##_long(_ofs, MACH_MSG_TYPE_INTEGER_8, 8,         \
                                     signed8_t, _num, _inl);
...
#define flick_mach3mig_array_decode_signed16_type(_ofs, _num, _inl, _long)     \
  flick_mach3mig_array_decode##_long(_ofs, MACH_MSG_TYPE_INTEGER_16, 16,       \
                                     signed16_t, _num, _inl);
...
#define flick_mach3mig_array_decode_signed32_type(_ofs, _num, _inl, _long)     \
  flick_mach3mig_array_decode##_long(_ofs, MACH_MSG_TYPE_INTEGER_32, 32,       \
                                     signed32_t, _num, _inl);
...
/* Array elements.  */
#define flick_mach3mig_array_decode_new_glob(max_size)                         \
  flick_mach3mig_decode_new_glob(max_size)
#define flick_mach3mig_array_decode_end_glob(max_size)                         \
  flick_mach3mig_decode_end_glob(max_size)
#define flick_mach3mig_array_decode_new_chunk(size)                            \
  flick_mach3mig_decode_new_chunk(size)
#define flick_mach3mig_array_decode_end_chunk(size)                            \
  flick_mach3mig_decode_end_chunk(size)
...
#define flick_mach3mig_array_decode_boolean(_ofs, _data)                       \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_BOOLEAN, 32,     \
                                   signed32_t);
#define flick_mach3mig_array_decode_char8(_ofs, _data)                         \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_CHAR, 8,         \
                                   signed8_t);
#define flick_mach3mig_array_decode_char16(_ofs, _data)                        \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 8,   \
                                   signed16_t);
#define flick_mach3mig_array_decode_signed8(_ofs, _data)                       \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,    \
                                   signed8_t);
#define flick_mach3mig_array_decode_unsigned8(_ofs, _data)                     \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_8, 8,    \
                                   unsigned8_t);
#define flick_mach3mig_array_decode_signed16(_ofs, _data)                      \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,  \
                                   signed16_t);
#define flick_mach3mig_array_decode_unsigned16(_ofs, _data)                    \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,  \
                                   unsigned16_t);
#define flick_mach3mig_array_decode_signed32(_ofs, _data)                      \
  flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_32, 32,  \
                                   signed32_t);
...
#define flick_mach3mig_array_decode_port(_ofs, _data, _adjust)                 \
  {                                                                            \
    flick_mach3mig_array_decode_prim(_ofs, _data, MACH_MSG_TYPE_PORT_SEND, 32, \
                                     mach_port_t);                             \
    if (_adjust != 1) {                                                        \
      kern_return_t res = mach_port_mod_refs(                                  \
          mach_task_self(), (_data), MACH_PORT_RIGHT_SEND, _adjust - 1);       \
    }                                                                          \
  }
...
#define flick_mach3mig_rpc_macro(iscomplex)                                    \
  {                                                                            \
    ...
    if (_err = flick_mach3mig_rpc(&_desc.d))                                   \
      return _err;                                                             \
  }
