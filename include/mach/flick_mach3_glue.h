...
struct flick_mach3_rpc_desc {
  ...
  /* Before calling flick_mach3_rpc(),
     ...
     marshaled. It always starts marshaling at offset 4*4, to leave room for a
     mach_msg_header (which is actually 6*4 bytes, but overwrites the 2*4-byte
     marshaled IDL ID).  */
...
#define flick_mach3_encode_signed16(_ofs, _data)                               \
  flick_mach3_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,           \
                          signed16_t);
#define flick_mach3_encode_unsigned16(ofs, data)                               \
  flick_mach3_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,           \
                          unsigned16_t);
#define flick_mach3_encode_signed32(_ofs, _data)                               \
  flick_mach3_encode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_32, 32,           \
                          signed32_t);
...
#define flick_mach3_decode_new_chunk(size)                                     \
  {                                                                            \
    flick_iftypecheck(if (_desc.d.rcv_ofs + (size) > _d_msgsize)               \
                          XXX throw MIG_TYPE_ERROR;);                          \
    _d_chunk = _desc.d.msg_buf + _desc.d.rcv_ofs;                              \
  }
#define flick_mach3_decode_end_chunk(size) _desc.d.rcv_ofs += (size);
...
#define flick_mach3_decode_signed16(_ofs, _data)                               \
  flick_mach3_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,           \
                          signed16_t);
#define flick_mach3_decode_unsigned16(_ofs, _data)                             \
  flick_mach3_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_16, 16,           \
                          unsigned16_t);
#define flick_mach3_decode_signed32(_ofs, _data)                               \
  flick_mach3_decode_prim(_ofs, _data, MACH_MSG_TYPE_INTEGER_32, 32,           \
                          signed32_t);
...
#define flick_mach3_decode_port(_ofs, _data, _adjust)                          \
  {                                                                            \
    flick_mach3_decode_prim(_ofs, _data, MACH_MSG_TYPE_PORT_SEND, 32,          \
                            mach_port_t);                                      \
    if (_adjust != 1) {                                                        \
      kern_return_t res = mach_port_mod_refs(                                  \
          mach_task_self(), (_data), MACH_PORT_RIGHT_SEND, _adjust - 1);       \
...
mach_msg_return_t flick_mach3_rpc(struct flick_mach3_rpc_desc *rpc,
                                  mach_port_t send_target,
                                  mach_msg_bits_t send_msgh_bits);

#define flick_mach3_rpc_macro(iscomplex)                                       \
  {                                                                            \
    kern_return_t result =                                                     \
        flick_mach3_rpc(&_desc.d, _msg_request /*XXX*/,                        \
