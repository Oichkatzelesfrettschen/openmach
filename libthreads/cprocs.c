...
private mach_msg_header_t wakeup_msg; /* prebuilt message used by idle
                                         threads */
/**
 * @brief cthread_kernel_limit function.
 */
int cthread_kernel_limit(void) { return cthread_max_kernel_threads; }
/**
 * @brief cthread_set_kernel_limit function.
 */
void cthread_set_kernel_limit(int n) { cthread_max_kernel_threads = n; }
/**
 * @brief cthread_wire function.
 */
void cthread_wire(void) {
  ...
  if (p->wired == MACH_PORT_NULL) {
    MACH_CALL(mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE,
                                 &p->wired),
              r);
    MACH_CALL(mach_port_insert_right(mach_task_self(), p->wired, p->wired,
...
/**
 * @brief cthread_unwire function.
 */
void cthread_unwire(void) {
  register cproc_t p = cproc_self();
  kern_return_t r;
...
/**
 * @brief cproc_alloc function.
 */
private cproc_t cproc_alloc(void) {
  ...
  return p;
/**
 * @brief cproc_init function.
 */
vm_offset_t cproc_init(void) {
  kern_return_t r;
  ...
  MACH_CALL(
      mach_port_allocate(mach_task_self(), MACH_PORT_RIGHT_RECEIVE, &wait_port),
      r);
  MACH_CALL(mach_port_insert_right(mach_task_self(), wait_port, wait_port,
                                   MACH_MSG_TYPE_MAKE_SEND),
            r);
...
private boolean_t cproc_ready(register cproc_t p, register int preq) {
  ...
  if (p->wired != MACH_PORT_NULL) {
    r = mach_msg(&p->msg, MACH_SEND_MSG, sizeof p->msg, 0, MACH_PORT_NULL,
  ...
  if ((ready_count > 0) && wait_count) {
    ...
    ready_count--;
    spin_unlock(&ready_lock);
    r = mach_msg(&wakeup_msg, MACH_SEND_MSG, sizeof wakeup_msg, 0,
  ...
  return FALSE;
/**
 * @brief cproc_waiting function.
 */
void cproc_waiting(register cproc_t p) {
...
#endif /* defined(STATISTICS) */
  for (;;) {
    MACH_CALL(mach_msg(&msg, MACH_RCV_MSG, 0, sizeof msg, wait_port,
                       MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL),
              r);
    spin_lock(&ready_lock);
...
/**
 * @brief cproc_waiter function.
 */
private cproc_t cproc_waiter(void) {
  ...
  if (waiter == NO_CPROC) {
    ...
    waiter = cproc_alloc();
    MACH_CALL(
        vm_allocate(mach_task_self(), &base, cthread_wait_stack_size, TRUE), r);
    waiter->stack_base = base;
    waiter->stack_size = cthread_wait_stack_size;
  }
  return (waiter);
private void cproc_block(void) {
  ...
  if (p->wired != MACH_PORT_NULL) {
    ...
    spin_unlock(&p->lock);
    MACH_CALL(mach_msg(&msg, MACH_RCV_MSG, 0, sizeof msg, p->wired,
                       MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL),
              r);
    return;
  }
...
/**
 * @brief cproc_create function.
 */
cproc_t cproc_create(void) {
  ...
  if (cthread_max_kernel_threads == 0 ||
    ...
    cproc_prepare(child, &child->context, cproc_stack_base(child, 0));
    cproc_ready(child, 0);
  }
  return child;
/**
 * @brief condition_wait function.
 */
void condition_wait(register condition_t c, mutex_t m) {
  register cproc_t p = cproc_self();
  ...
  mutex_lock(m);
/**
 * @brief cond_signal function.
 */
void cond_signal(register condition_t c) {
  register cproc_t p;
...
/**
 * @brief cond_broadcast function.
 */
void cond_broadcast(register condition_t c) {
  register cproc_t p;
...
/**
 * @brief mutex_lock_solid function.
 */
void mutex_lock_solid(register mutex_t m) {
  register cproc_t p = cproc_self();
  register int queued;
  ...
  while (1) {
    ...
    if (spin_try_lock(&m->held)) {
      if (queued)
        cthread_queue_deq(&m->queue, cproc_t, p);
      spin_unlock(&m->lock);
#if defined(WAIT_DEBUG)
      p->waiting_for = (char *)0;
      p->wait_type = CTW_NONE;
...
/**
 * @brief mutex_unlock_solid function.
 */
void mutex_unlock_solid(register mutex_t m) {
  register cproc_t new;
  if (!spin_try_lock(&m->held))
    return;
  spin_lock(&m->lock);
...
private port_entry_t get_port_entry(mach_port_t port, int min, int max) {
  ...
  return i;
/**
 * @brief cthread_msg_busy function.
 */
...
/**
 * @brief cthread_msg_active function.
 */
...
mach_msg_return_t cthread_mach_msg(register mach_msg_header_t *header,
                                   register mach_msg_option_t option,
                                   mach_msg_size_t send_size,
                                   mach_msg_size_t rcv_size,
                                   register mach_port_t rcv_name,
                                   mach_msg_timeout_t timeout,
                                   mach_port_t notify, int min, int max) {
  register port_entry_t port_entry;
  register cproc_t p = cproc_self();
  register int sent = 0;
  mach_msg_return_t r;
  ...
  if (!(port_entry == (port_entry_t)p->busy)) {
    if (port_entry->held >= max) {
      if (option & MACH_SEND_MSG) {
        spin_unlock(&port_entry->lock);
        r = mach_msg(header, option & ~MACH_RCV_MSG, send_size, 0,
                     MACH_PORT_NULL, timeout, notify);
        if (r != MACH_MSG_SUCCESS)
          return r;
        spin_lock(&port_entry->lock);
        sent = 1;
  ...
  if ((option & MACH_SEND_MSG) && !sent) {
    r = mach_msg(header, option, send_size, rcv_size, rcv_name, timeout,
                 notify);
  } else {
    r = mach_msg(header, option & ~MACH_SEND_MSG, 0, rcv_size, rcv_name,
                 timeout, notify);
  }
  return r;
/**
 * @brief cproc_fork_prepare function.
 */
void cproc_fork_prepare(void) {
  register cproc_t p = cproc_self();
  vm_inherit(mach_task_self(), p->stack_base, p->stack_size, VM_INHERIT_COPY);
  spin_lock(&port_lock);
  spin_lock(&cproc_list_lock);
/**
 * @brief cproc_fork_parent function.
 */
void cproc_fork_parent(void) {
  register cproc_t p = cproc_self();
...
/**
 * @brief cproc_fork_child function.
 */
void cproc_fork_child(void) {
