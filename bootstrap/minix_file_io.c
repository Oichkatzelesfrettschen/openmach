...
register struct file *fp;
{
  ...
  rc = device_read(fp->f_dev, 0, (recnum_t)minix_fsbtodb(fp->f_fs, disk_block),
                   (int)MINIX_BLOCK_SIZE, (char **)&buf, &buf_size);
  if (rc != KERN_SUCCESS)
    return (rc);
  ...
  for (; level >= 0; level--) {
    ...
    if (fp->f_blkno[level] == ind_block_num) {
      ...
      rc = device_read(fp->f_dev, 0,
                       ...
                       MINIX_BLOCK_SIZE, (char **)&data, &size);
      if (rc != KERN_SUCCESS)
        return (rc);
...
vm_size_t *size_p;  /* out */
{
  ...
  if (((daddr_t)file_block) != fp->f_buf_blkno) {
    ...
    if (disk_block == 0) {
      ...
      rc = device_read(fp->f_dev, 0, (recnum_t)minix_fsbtodb(fs, disk_block),
...
ino_t *inumber_p; /* out */
{
  ...
  while (offset < fp->i_ic.i_size) {
    ...
    if (dp->inode != 0) {
      ...
      if (strlen(tmp_name) == length && !strcmp(name, tmp_name)) {
        /* found entry */
        *inumber_p = dp->inode;
        return (0);
      }
...
static void unmount_fs(fp) register struct file *fp;
{
  if (file_is_structured(fp)) {
    (void)vm_deallocate(mach_task_self(), (vm_offset_t)fp->f_fs, MINIX_SBSIZE);
    fp->f_fs = 0;
  }
}
 ...
 * Open a file.
 */
int minix_open_file(master_device_port, path, fp)
 ...
 */
void minix_close_file(fp) register struct file *fp;
{
  register int i;
...
int minix_file_is_directory(struct file *fp) {
  return (fp->i_ic.i_mode & IFMT) == IFDIR;
}

int minix_file_is_regular(struct file *fp) {
 ...
 */
int minix_read_file(fp, offset, start, size, resid)
register struct file *fp;
vm_offset_t offset;
vm_offset_t start;
vm_size_t size;
 ...
 */
int minix_open_file_direct(dev, fdp, is_structured)
mach_port_t dev;
...
boolean_t is_structured;
{
  ...
  (void)vm_deallocate(mach_task_self(), (vm_offset_t)fs, MINIX_SBSIZE);
 ...
 */
int minix_add_file_direct(fdp, fp)
register struct file_direct *fdp;
register struct file *fp;
{
  register struct minix_super_block *fs;
...
int minix_remove_file_direct(fdp)
 ...
 */
int minix_page_read_file_direct(fdp, offset, size, addr, size_read)
register struct file_direct *fdp;
vm_offset_t offset;
vm_size_t size;
 ...
 */
int minix_page_write_file_direct(fdp, offset, addr, size, size_written)
register struct file_direct *fdp;
vm_offset_t offset;
vm_offset_t addr;
vm_size_t size;
