...
void set_partition_of(x, p) int x;
partition_t p;
{
  if (x >= all_partitions.n_partitions || x < 0)
 ...
 * size is in BYTES.
 */
void create_paging_partition(name, size, p_read, p_write, p_private,
                             isa_file) char *name;
vm_size_t size;
int (*p_read)();
int (*p_write)();
...
boolean_t isa_file;
{
  ...
  mutex_lock(&all_partitions.lock);
  {
    ...
    if (i == all_partitions.n_partitions) {
      ...
      n = i ? (i << 1) : 2;
      new_list = (partition_t *)kalloc(n * sizeof(partition_t));
      if (new_list == 0)
 ...
 */
p_index_t choose_partition(size, cur_part)
unsigned int size;
 ...
 */
vm_offset_t pager_alloc_page(pindex, lock_it)
p_index_t pindex;
{
  register int bm_e;
  register int bit;
 ...
 */
void pager_dealloc_page(pindex, page, lock_it) p_index_t pindex;
...
union dp_map {
  ...
  struct {
    unsigned int p_offset : 24, p_index : 8;
  } block;
...
#define INDIRECT_PAGEMAP(size) (size > PAGEMAP_ENTRIES)
 ...
 */
void pager_alloc(pager, part, size) register dpager_t pager;
p_index_t part;
...
#ifdef CHECKSUM
  if (INDIRECT_PAGEMAP(size)) {
    mapptr = (vm_offset_t *)kalloc(INDIRECT_PAGEMAP_SIZE(size));
...
vm_size_t pager_allocated(pager)
register dpager_t pager;
{
  ...
  if (INDIRECT_PAGEMAP(size)) {
    for (emap = &map[INDIRECT_PAGEMAP_ENTRIES(size)]; map < emap; map++) {
      ...
      for (emap2 = &map2[PAGEMAP_ENTRIES]; map2 < emap2; map2++)
        if (!no_block(*map2))
          asize++;
    }
  } else {
...
unsigned int pager_pages(pager, pages, numpages)
dpager_t pager;
 ...
 * system memory.
 */
void pager_extend(pager, new_size) register dpager_t pager;
register vm_size_t new_size; /* in pages */
{
  ...
  if (INDIRECT_PAGEMAP(old_size)) {
     ...
     */
    new_mapptr = (dp_map_t)kalloc(INDIRECT_PAGEMAP_SIZE(new_size));
    ...
    pager->size = new_size;
#ifdef CHECKSUM
    new_mapptr = (vm_offset_t *)kalloc(INDIRECT_PAGEMAP_SIZE(new_size));
  ...
  pager->size = new_size;
#ifdef CHECKSUM
  new_mapptr = (vm_offset_t *)kalloc(PAGEMAP_SIZE(new_size));
  old_mapptr = pager->checksum;
  for (i = 0; i < old_size; i++)
    new_mapptr[i] = old_mapptr[i];
  for (; i < new_size; i++)
 ...
 */
union dp_map pager_read_offset(pager, offset)
register dpager_t pager;
vm_offset_t offset;
{
 ...
 */
pager_release_offset(pager, offset) register dpager_t pager;
vm_offset_t offset;
{
 ...
 */
union dp_map pager_move_page(block)
union dp_map block;
{
  ...
  rc = (*old_part->p_read)(old_part->p_private, offset, vm_page_size, &raddr,
                           &size);
  if (rc != 0)
    panic(here, my_name);
  ...
  rc = (*new_part->p_write)(new_part->p_private, offset, raddr, size, &size);
  if (rc != 0)
    panic(here, my_name);
 ...
 */
int pager_get_checksum(pager, offset)
register dpager_t pager;
vm_offset_t offset;
{
  register vm_offset_t f_page;
 ...
 */
int pager_put_checksum(pager, offset, checksum)
register dpager_t pager;
vm_offset_t offset;
int checksum;
{
 ...
 */
int compute_checksum(addr, size)
vm_offset_t addr;
vm_size_t size;
{
 ...
 * without notice!
 */
union dp_map pager_write_offset(pager, offset)
register dpager_t pager;
vm_offset_t offset;
{
  register vm_offset_t f_page;
  ...
  if (INDIRECT_PAGEMAP(pager->size)) {
    ...
    if (mapptr == 0) {
...
#ifdef CHECKSUM
      {
        ...
        cksumptr = (vm_offset_t *)kalloc(PAGEMAP_SIZE(PAGEMAP_ENTRIES));
        if (cksumptr == 0) {
          /* out of space! */
          no_paging_space(TRUE);
          goto out;
        }
        pager->checksum[f_page / PAGEMAP_ENTRIES] = (vm_offset_t)cksumptr;
 ...
 */
void pager_dealloc(pager) register dpager_t pager;
{
  register int i, j;
  register dp_map_t mapptr;
  register union dp_map block;
  ...
  if (INDIRECT_PAGEMAP(pager->size)) {
    for (i = INDIRECT_PAGEMAP_ENTRIES(pager->size); --i >= 0;) {
      ...
      if (mapptr != 0) {
        for (j = 0; j < PAGEMAP_ENTRIES; j++) {
          block = mapptr[j];
          if (!no_block(block))
            pager_dealloc_page(block.block.p_index, block.block.p_offset, TRUE);
        }
    ...
    kfree((char *)pager->checksum, INDIRECT_PAGEMAP_SIZE(pager->size));
#endif CHECKSUM
  } else {
    mapptr = pager->map;
 ...
 */
boolean_t pager_realloc(pager, pindex)
register dpager_t pager;
p_index_t pindex;
{
  register dp_map_t map, emap;
 ...
 */
int default_read(ds, addr, size, offset, out_addr, deallocate)
...
boolean_t deallocate;
{
  ...
  *out_addr = addr;
  ...
  do {
    rc = (*part->p_read)(part->p_private, offset, size, &raddr, &rsize);
    if (rc != 0)
      return (PAGER_ERROR);
...
#ifdef CHECKSUM
  {
    int write_checksum, read_checksum;
    ...
    if (write_checksum != read_checksum) {
      panic("PAGER CHECKSUM ERROR: offset 0x%x, written 0x%x, read 0x%x",
...
int default_write(ds, addr, size, offset)
register dpager_t ds;
...
register vm_offset_t offset;
{
   ...
   */
  do {
    rc = (*part->p_write)(part->p_private, offset, addr, size, &wsize);
    if (rc != 0) {
...
boolean_t default_has_page(ds, offset)
dpager_t ds;
vm_offset_t offset;
...
struct dstruct {
  ...
  struct condition waiting_seqno, /* someone waiting on seqno */
...
void pager_port_list_insert(port, ds) mach_port_t port;
default_pager_t ds;
{
...
#define pager_port_lookup(_port_)                                              \
  ((!MACH_PORT_VALID(_port_) ||                                                \
    ((default_pager_t)dnameof(_port_))->pager != (_port_))                     \
       ? DEFAULT_PAGER_NULL                                                    \
       : (default_pager_t)dnameof(_port_))

void pager_port_list_delete(ds) default_pager_t ds;
{
  mutex_lock(&all_pagers.lock);
 ...
 */
kern_return_t destroy_paging_partition(name, pp_private)
char *name;
char **pp_private;
{
 ...
 */
void pager_port_lock(ds, seqno) default_pager_t ds;
 ...
 */
void pager_port_unlock(ds) default_pager_t ds;
{
  ds->seqno++;
  dstruct_unlock(ds);
 ...
 */
void pager_port_start_read(ds) default_pager_t ds;
{ ds->readers++; }

/*
 * Wait for readers.  Unlocks and relocks pager if wait needed.
 */
void pager_port_wait_for_readers(ds) default_pager_t ds;
{
  while (ds->readers != 0) {
 ...
 */
void pager_port_finish_read(ds) default_pager_t ds;
{
  dstruct_lock(ds);
  if (--ds->readers == 0) {
    dstruct_unlock(ds);
 ...
 */
void pager_port_start_write(ds) default_pager_t ds;
{ ds->writers++; }

/*
 * Wait for writers.  Unlocks and relocks pager if wait needed.
 */
void pager_port_wait_for_writers(ds) default_pager_t ds;
{
  while (ds->writers != 0) {
 ...
 */
void pager_port_finish_write(ds) default_pager_t ds;
{
  dstruct_lock(ds);
  if (--ds->writers == 0) {
    dstruct_unlock(ds);
 ...
 */
void pager_port_wait_for_refs(ds) default_pager_t ds;
{
 ...
 */
void pager_port_finish_refs(ds) default_pager_t ds;
 ...
 */
void pager_port_check_request(ds, pager_request) default_pager_t ds;
...
void default_pager_add(ds, internal) default_pager_t ds;
boolean_t internal;
{
  ...
  kr = mach_port_request_notification(default_pager_self, pager,
                                      MACH_NOTIFY_NO_SENDERS, sync, pager,
 ...
 */
kern_return_t seqnos_memory_object_create(old_pager, seqno, new_pager, new_size,
...
rename_it:
  kr =
      mach_port_rename(default_pager_self, new_pager, (mach_port_t)pnameof(ds));
  if (kr != KERN_SUCCESS) {
    default_pager_t ds1;
...
kern_return_t seqnos_memory_object_init(pager, seqno, pager_request, pager_name,
                                        pager_page_size)
mach_port_t pager;
mach_port_seqno_t seqno;
mach_port_t pager_request;
...
vm_size_t pager_page_size;
{
  ...
  kr = memory_object_set_attributes(pager_request, TRUE,
                                    FALSE, /* do not cache */
...
kern_return_t seqnos_memory_object_terminate(pager, seqno, pager_request,
                                             pager_name)
mach_port_t pager;
mach_port_seqno_t seqno;
mach_port_t pager_request;
mach_port_t pager_name;
{
  ...
  kr = mach_port_mod_refs(default_pager_self, pager_name, MACH_PORT_RIGHT_SEND,
                          -name_refs);
  if (kr != KERN_SUCCESS)
    panic(here, my_name);
...
void default_pager_no_senders(pager, seqno, mscount) memory_object_t pager;
mach_port_seqno_t seqno;
mach_port_mscount_t mscount;
{
  ...
  kr = mach_port_mod_refs(default_pager_self, pager, MACH_PORT_RIGHT_RECEIVE,
                          -1);
  if (kr != KERN_SUCCESS)
    panic(here, my_name);
...
kern_return_t seqnos_memory_object_data_request(pager, seqno, reply_to, offset,
...
vm_prot_t protection_required;
{
  ...
  if (errors) {
    ...
    (void)memory_object_data_error(reply_to, offset, vm_page_size,
                                   KERN_FAILURE);
    goto done;
  }

  rc = default_read(&ds->dpager, dpt->dpt_buffer, vm_page_size, offset, &addr,
  ...
  case PAGER_SUCCESS:
    if (addr != dpt->dpt_buffer) {
       ...
       */
      (void)memory_object_data_supply(reply_to, offset, addr, vm_page_size,
                                      TRUE, VM_PROT_NONE, FALSE,
                                      MACH_PORT_NULL);
    } else {
      (void)memory_object_data_provided(reply_to, offset, addr, vm_page_size,
                                        VM_PROT_NONE);
    }
    break;

  case PAGER_ABSENT:
    (void)memory_object_data_unavailable(reply_to, offset, vm_page_size);
    break;
 ...
 */
kern_return_t seqnos_memory_object_data_initialize(pager, seqno, pager_request,
...
vm_size_t data_cnt;
{
  ...
  for (amount_sent = 0; amount_sent < data_cnt; amount_sent += vm_page_size) {

    if (!default_has_page(&ds->dpager, offset + amount_sent)) {
      if (default_write(&ds->dpager, addr + amount_sent, vm_page_size,
                        offset + amount_sent) != PAGER_SUCCESS) {
 ...
 */
kern_return_t seqnos_memory_object_data_write(pager, seqno, pager_request,
                                              offset, addr, data_cnt)
memory_object_t pager;
mach_port_seqno_t seqno;
mach_port_t pager_request;
...
vm_size_t data_cnt;
{
  register vm_size_t amount_sent;
  default_pager_t ds;
  ...
  for (amount_sent = 0; amount_sent < data_cnt; amount_sent += vm_page_size) {
    ...
    result = default_write(&ds->dpager, addr + amount_sent, vm_page_size,
                           offset + amount_sent);
    if (result != KERN_SUCCESS) {
#if debug
      printf("%s WRITE ERROR on default_pageout:", my_name);
      printf(" pager=%x, offset=0x%x, length=0x%x, result=%d\n", pager,
             offset + amount_sent, vm_page_size, result);
#endif
      dstruct_lock(ds);
      ds->errors++;
      dstruct_unlock(ds);
...
/*ARGSUSED*/
kern_return_t seqnos_memory_object_copy(old_memory_object, seqno,
                                        old_memory_control, offset, length,
                                        new_memory_object)
...
kern_return_t seqnos_memory_object_lock_completed(pager, seqno, pager_request,
                                                  offset, length)
memory_object_t pager;
mach_port_seqno_t seqno;
mach_port_t pager_request;
...
kern_return_t seqnos_memory_object_data_unlock(pager, seqno, pager_request,
                                               offset, addr, data_cnt)
memory_object_t pager;
mach_port_seqno_t seqno;
mach_port_t pager_request;
...
kern_return_t seqnos_memory_object_supply_completed(pager, seqno, pager_request,
...
kern_return_t seqnos_memory_object_data_return(pager, seqno, pager_request,
                                               offset, addr, data_cnt, dirty,
                                               kernel_copy)
memory_object_t pager;
mach_port_seqno_t seqno;
mach_port_t pager_request;
...
kern_return_t seqnos_memory_object_change_completed(pager, seqno, may_cache,
                                                    copy_strategy)
memory_object_t pager;
mach_port_seqno_t seqno;
boolean_t may_cache;
...
boolean_t default_pager_demux_object(in, out)
...
boolean_t default_pager_demux_default(in, out)
mach_msg_header_t *in;
mach_msg_header_t *out;
{
...
void default_pager_thread_privileges() {
  /*
...
any_t default_pager_thread(arg)
any_t arg;
{
  ...
  for (;;) {
    kr = mach_msg_server(default_pager_demux_object,
                         default_pager_msg_size_object, pset);
    panic(my_name, kr);
  }
}

void start_default_pager_thread(internal) boolean_t internal;
{
  default_pager_thread_t *dpt;
  kern_return_t kr;
  ...
  kr = vm_allocate(default_pager_self, &dpt->dpt_buffer, vm_page_size, TRUE);
  if (kr != KERN_SUCCESS)
    panic(my_name);
  wire_memory(dpt->dpt_buffer, vm_page_size, VM_PROT_READ | VM_PROT_WRITE);

  dpt->dpt_thread = cthread_fork(default_pager_thread, (any_t)dpt);
}

void default_pager_initialize(host_port) mach_port_t host_port;
{
  memory_object_t DMM;
  kern_return_t kr;
  ...
  if (default_pager_internal_count == 0) {
    ...
    (void)host_info(host_port, HOST_BASIC_INFO, (host_info_t)&h_info,
                    &h_info_count);
...
void default_pager() {
  ...
  kr = mach_port_move_member(default_pager_self, default_pager_default_port,
                             default_pager_default_set);
  if (kr != KERN_SUCCESS)
    panic(my_name);

  kr = mach_port_move_member(default_pager_self, default_pager_exception_port,
                             default_pager_default_set);
  if (kr != KERN_SUCCESS)
    panic(my_name);

  kr = mach_port_move_member(default_pager_self, default_pager_bootstrap_port,
                             default_pager_default_set);
  if (kr != KERN_SUCCESS)
    panic(my_name);
...
rename_it:
  ...
  result = mach_port_allocate_name(default_pager_self, MACH_PORT_RIGHT_RECEIVE,
                                   port);
  if (result != KERN_SUCCESS) {
    default_pager_t ds1;
...
natural_t *pcountp;
{
  ...
  queue_iterate(&all_pagers.queue, entry, default_pager_t, links) {
    ...
    if ((num_pagers >= opotential) || (num_pagers >= ppotential)) {
      /*
    ...
    if (--entry->name_refs == 0) {
      ...
      kr = mach_port_mod_refs(default_pager_self, port, MACH_PORT_RIGHT_SEND,
  ...
  if (objects == *objectsp) {
    ...
    *ocountp = 0;
    ...
    if (used != osize)
      (void)vm_deallocate(default_pager_self, oaddr + used, osize - used);
  ...
  if (ports == *portsp) {
    ...
    *pcountp = 0;
    ...
    if (used != psize)
      (void)vm_deallocate(default_pager_self, paddr + used, psize - used);
...
kern_return_t default_pager_object_pages(pager, object, pagesp, countp)
mach_port_t pager;
mach_port_t object;
...
natural_t *countp;
{
  ...
  if (pages == *pagesp) {
    ...
    *countp = 0;
    ...
    if (used != size)
      (void)vm_deallocate(default_pager_self, addr + used, size - used);
...
kern_return_t default_pager_paging_file(pager, mdport, file_name, add)
mach_port_t pager;
mach_port_t mdport;
...
#endif
  if (add) {
    kr = add_paging_file(bootstrap_master_device_port, file_name);
  } else {
...
default_pager_register_fileserver(pager, fileserver) mach_port_t pager;
mach_port_t fileserver;
{
  ...
  if (0)
    dp_helper_paging_space(0, 0, 0); /*just linkit*/
#endif
  return KERN_SUCCESS;
}
 ...
 */
no_paging_space(out_of_memory) boolean_t out_of_memory;
{
...
overcommitted(got_more_space, space) boolean_t got_more_space;
...
paging_space_info(totp, freep) vm_size_t *totp, *freep;
{
...
kern_return_t catch_exception_raise(exception_port, thread, task, exception,
                                    code, subcode)
...
kern_return_t do_bootstrap_privileged_ports(bootstrap, hostp, devicep)
...
void bootstrap_compat(in, out) mach_msg_header_t *in, *out;
{
  ...
  mr = mach_msg(&imsg.hdr, MACH_SEND_MSG, sizeof imsg, 0, MACH_PORT_NULL,
                MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
  if (mr != MACH_MSG_SUCCESS)
    (void)mach_port_deallocate(default_pager_self, imsg.hdr.msgh_remote_port);
 ...
 */
int set_ras_address(basepc, boundspc)
vm_offset_t basepc;
