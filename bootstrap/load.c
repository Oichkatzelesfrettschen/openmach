...
char *set_regs(mach_port_t user_task, mach_port_t user_thread,
               struct exec_info *info, int arg_size);

static void read_symtab_from_file(struct file *fp, mach_port_t host_port,
                                  task_t task, char *symtab_name,
                                  struct stuff *st);

/* Callback functions for reading the executable file.  */
static int prog_read(void *handle, vm_offset_t file_ofs, void *buf,
...
static int prog_read_exec(void *handle, vm_offset_t file_ofs,
  ...
  printf("section %08x-%08x-%08x prot %08x (%08x-%08x)\n",
          mem_addr, mem_addr+file_size, mem_addr+mem_size, mem_prot, page_start,
  page_end);
  */

  result =
      vm_allocate(mach_task_self(), &area_start, page_end - page_start, TRUE);
  if (result)
    return (result);
  ...
  result =
      vm_allocate(st->user_task, &page_start, page_end - page_start, FALSE);
  if (result)
    return (result);
  assert(page_start == trunc_page(mem_addr));

  result =
      vm_write(st->user_task, page_start, area_start, page_end - page_start);
  if (result)
    return (result);
  ...
  if (load_protect_text && (mem_prot != VM_PROT_ALL)) {
    result = vm_protect(st->user_task, page_start, page_end - page_start, FALSE,
                        mem_prot);
    if (result)
      return (result);
  }
...
int boot_load_program(mach_port_t master_host_port,
                      mach_port_t master_device_port, task_t user_task,
                      thread_t user_thread, char *server_dir_name,
                      ...) /* first component is program name */
{
   ...
   */
  (void)strbuild(namebuf, server_dir_name, "/", file_name, (char *)0);
   ...
   */
  {
    ...
    arg_page_size =
        (vm_size_t)(round_page(u_arg_start + arg_len) - u_arg_page_start);

    result = vm_allocate(mach_task_self(), &k_arg_page_start,
                         (vm_size_t)arg_page_size, TRUE);
    if (result)
    ...
    u_cp = (char *)u_arg_start + arg_count * sizeof(char *) +
           2 * sizeof(char *) + sizeof(integer_t);
    ...
    *k_ap = (char *)0;
     ...
     */
    (void)vm_write(user_task, u_arg_page_start, k_arg_page_start,
                   arg_page_size);

    (void)vm_deallocate(mach_task_self(), k_arg_page_start, arg_page_size);
  }
...
static void read_symtab_from_file(struct file *fp, mach_port_t host_port,
   ...
   */
  table_size = sizeof(vm_size_t) + st->aout_symtab_size + st->aout_strtab_size;
  ...
  *(vm_size_t *)symtab = st->aout_symtab_size;
  result = read_file(fp, st->aout_symtab_ofs, symtab + sizeof(vm_size_t),
                     st->aout_symtab_size + st->aout_strtab_size, &resid);
  if (result || resid) {
     ...
     */
    result = host_load_symbol_table(host_port, task, symtab_name, symtab,
                                    table_size);
  }
