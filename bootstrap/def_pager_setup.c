...
kern_return_t add_paging_file(master_device_port, file_name)
...
char *file_name;
{
   ...
   */
  create_paging_partition(file_name, fdp->fd_size * fdp->fd_bsize,
                          page_read_file_direct, page_write_file_direct,
                          (char *)fdp, isa_file);
 ...
 */
kern_return_t remove_paging_file(file_name)
char *file_name;
{
...
boolean_t default_pager_setup(master_device_port, server_dir_name)
mach_port_t master_device_port;
char *server_dir_name;
{
  ...
  (void)strbuild(paging_file_name, server_dir_name, "/paging_file", (char *)0);
