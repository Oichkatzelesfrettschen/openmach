...
vm_offset_t kget_space(vm_offset_t size) {
  ...
  while (kalloc_next_space + size > kalloc_end_of_space) {
    ...
    if (new_space == 0) {
      ...
      if (vm_map(mach_task_self(), &new_space, space_to_add, (vm_offset_t)0,
                 TRUE, MEMORY_OBJECT_NULL, (vm_offset_t)0, FALSE,
                 VM_PROT_DEFAULT, VM_PROT_ALL,
                 VM_INHERIT_DEFAULT) != KERN_SUCCESS)
        return 0;
      wire_memory(new_space, space_to_add, VM_PROT_READ | VM_PROT_WRITE);
      spin_lock(&kget_space_lock);
      continue;
    }
    ...
    if (new_space != kalloc_end_of_space) {
       ...
       * and start a new one.
       */
      kalloc_wasted_space += kalloc_end_of_space - kalloc_next_space;
...
void *kalloc(vm_size_t size) {
  ...
  if (allocsize < kalloc_max) {
    ...
    if (vm_allocate(mach_task_self(), &addr, allocsize, TRUE) != KERN_SUCCESS)
      addr = 0;
  }
  return (void *)addr;
}
...
void *malloc(vm_size_t size) { return (void *)kalloc(size); }

void *realloc(void *block, vm_size_t size) { panic("realloc not implemented"); }

void free(void *addr) { panic("free not implemented"); }
