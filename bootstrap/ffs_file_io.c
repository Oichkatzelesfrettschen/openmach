...
register struct file *fp;
{
  ...
  rc = device_read(fp->f_dev, 0, (recnum_t)fsbtodb(fp->f_fs, disk_block),
                   (int)fs->fs_bsize, (char **)&buf, &buf_size);
  if (rc != KERN_SUCCESS)
    return (rc);
  ...
  for (; level >= 0; level--) {
    ...
    if (fp->f_blkno[level] == ind_block_num) {
      ...
      rc = device_read(fp->f_dev, 0, (recnum_t)fsbtodb(fp->f_fs, ind_block_num),
                       fp->f_fs->fs_bsize, (char **)&data, &size);
      if (rc != KERN_SUCCESS)
        return (rc);
 ...
 * Open a file.
 */
int ffs_open_file(master_device_port, path, fp)
 ...
 */
void ffs_close_file(fp) register struct file *fp;
{
  register int i;
...
int ffs_file_is_directory(struct file *fp) {
  return (fp->i_mode & IFMT) == IFDIR;
}

int ffs_file_is_regular(struct file *fp) {
 ...
 */
int ffs_read_file(fp, offset, start, size, resid)
register struct file *fp;
vm_offset_t offset;
vm_offset_t start;
vm_size_t size;
 ...
 */
int ffs_open_file_direct(dev, fdp, is_structured)
mach_port_t dev;
 ...
 */
int ffs_add_file_direct(fdp, fp)
register struct file_direct *fdp;
register struct file *fp;
{
  register struct fs *fs;
  ...
  fs = fp->f_fs;
  if (fdp->fd_bsize != fs->fs_bsize || fdp->fd_fsbtodb != fs->fs_fsbtodb)
    return FS_INVALID_FS;
...
int ffs_remove_file_direct(fdp)
 ...
 */
int ffs_page_read_file_direct(fdp, offset, size, addr, size_read)
register struct file_direct *fdp;
vm_offset_t offset;
vm_size_t size;
 ...
 */
int ffs_page_write_file_direct(fdp, offset, addr, size, size_written)
register struct file_direct *fdp;
vm_offset_t offset;
vm_offset_t addr;
vm_size_t size;
