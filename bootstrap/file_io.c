...
*MINIX FS patches : Csizmazia Balazs, University ELTE,
    Hungary * /
...
#include "file_io.h"
        ...
        int open_file(master_device_port, path, fp)
...
struct file *fp;
{
  ...
  if ((rc = ext2_open_file(master_device_port, path, fp)) != FS_INVALID_FS) {
    if (rc == 0)
      fp->f_fstype = EXT2_FS;
    return rc;
  }
  if ((rc = minix_open_file(master_device_port, path, fp)) != FS_INVALID_FS) {
    if (rc == 0)
...
void close_file(fp) register struct file *fp;
{
  switch (fp->f_fstype) {
  case EXT2_FS:
    ext2_close_file(fp);
...
int read_file(fp, offset, start, size, resid)
...
int file_is_directory(struct file *f) {
  switch (f->f_fstype) {
  case EXT2_FS:
...
int file_is_regular(struct file *f) {
  switch (f->f_fstype) {
  case EXT2_FS:
...
int open_file_direct(dev, fdp, is_structured)
mach_port_t dev;
...
boolean_t is_structured;
{
  ...
  if ((rc = ext2_open_file_direct(dev, fdp, is_structured)) != FS_INVALID_FS) {
    if (rc == 0)
      fdp->f_fstype = EXT2_FS;
    return rc;
  }
  if ((rc = minix_open_file_direct(dev, fdp, is_structured)) != FS_INVALID_FS) {
    if (rc == 0)
...
int page_read_file_direct(fdp, offset, size, addr, size_read)
register struct file_direct *fdp;
vm_offset_t offset;
vm_size_t size;
...
int page_write_file_direct(fdp, offset, addr, size, size_written)
register struct file_direct *fdp;
vm_offset_t offset;
vm_offset_t addr;
vm_size_t size;
