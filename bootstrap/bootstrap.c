...
char **argv;
{
  ...
  (void)strbuild(server_dir_name, "/dev/", root_name, "/mach_servers",
                 (char *)0);
  ...
  default_pager_setup(bootstrap_master_device_port, server_dir_name);
  ...
  (void)task_set_exception_port(user_task, default_pager_exception_port);
  (void)task_set_bootstrap_port(user_task, default_pager_bootstrap_port);
   ...
   */
  result = boot_load_program(
      bootstrap_master_host_port, bootstrap_master_device_port, user_task,
      user_thread, server_dir_name, startup_name, flag_string, root_name,
      server_dir_name, (char *)0);
  if (result != 0)
...

  {
    ...
    kr = vm_region(my_task, &r_addr, &r_size, &r_protection, &r_max_protection,
                   &r_inheritance, &r_is_shared, &r_object_name, &r_offset);
