 ...
 */
static void free_file_buffers(fp) register struct file *fp;
{
  register int level;
  ...
  for (level = 0; level < NIADDR; level++) {
    if (fp->f_blk[level] != 0) {
      (void)vm_deallocate(mach_task_self(), fp->f_blk[level],
                          fp->f_blksize[level]);
      fp->f_blk[level] = 0;
    }
    fp->f_blkno[level] = -1;
   ...
   */
  if (fp->f_buf != 0) {
    (void)vm_deallocate(mach_task_self(), fp->f_buf, fp->f_buf_size);
    fp->f_buf = 0;
  }
  fp->f_buf_blkno = -1;
}
 ...
 */
static int read_inode(inumber, fp)
ino_t inumber;
register struct file *fp;
{
  ...
  rc = device_read(fp->f_dev, 0, (recnum_t)fsbtodb(fp->f_fs, disk_block),
                   (int)EXT2_BLOCK_SIZE(fs), (char **)&buf, &buf_size);
  if (rc != KERN_SUCCESS)
    return (rc);
 ...
 * contains that block.
 */
static int block_map(fp, file_block, disk_block_p)
struct file *fp;
daddr_t file_block;
daddr_t *disk_block_p; /* out */
{
  ...
  for (; level >= 0; level--) {
    ...
    if (fp->f_blkno[level] == ind_block_num) {
      ...
      rc = device_read(fp->f_dev, 0, (recnum_t)fsbtodb(fp->f_fs, ind_block_num),
                       EXT2_BLOCK_SIZE(fp->f_fs), (char **)&data, &size);
      if (rc != KERN_SUCCESS)
        return (rc);
  ...
  for (idx = 0; idx < NIADDR; idx++)
    if (oldsize[idx] != 0)
      (void)vm_deallocate(mach_task_self(), olddata[idx], oldsize[idx]);
 ...
 */
static int buf_read_file(fp, offset, buf_p, size_p)
register struct file *fp;
vm_offset_t offset;
...
vm_size_t *size_p;  /* out */
{
  ...
  if (file_block != fp->f_buf_blkno) {
    ...
    if (disk_block == 0) {
      (void)vm_allocate(mach_task_self(), &fp->f_buf, block_size, TRUE);
      fp->f_buf_size = block_size;
    } else {
      rc = device_read(fp->f_dev, 0, (recnum_t)fsbtodb(fs, disk_block),
                       (int)block_size, (char **)&fp->f_buf,
 ...
 * i_number.
 */
static int search_directory(name, fp, inumber_p)
char *name;
register struct file *fp;
ino_t *inumber_p; /* out */
{
  ...
  while (offset < fp->i_ic.i_size) {
    ...
    if (dp->inode != 0) {
      ...
      if (dp->name_len == length && !strcmp(name, tmp_name)) {
        /* found entry */
        *inumber_p = dp->inode;
        return (0);
      }
...
static int read_fs(dev, fsp, gdp, gd_size_p)
mach_port_t dev;
...
vm_size_t *gd_size_p;
{
   ...
   */
  error =
      device_read(dev, 0, (recnum_t)SBLOCK, SBSIZE, (char **)&buf, &buf_size);
  if (error)
    return (error);
  ...
  *fsp = fs;
  ...
  gd_blocks =
      (gd_count + EXT2_DESC_PER_BLOCK(fs) - 1) / EXT2_DESC_PER_BLOCK(fs);
   ...
   */
  error = device_read(dev, 0, (recnum_t)gd_sector, gd_size, (char **)&buf2,
                      &buf2_size);
  if (error) {
    (void)vm_deallocate(mach_task_self(), buf, buf_size);
    return error;
...
static void unmount_fs(fp) register struct file *fp;
{
  if (file_is_structured(fp)) {
    (void)vm_deallocate(mach_task_self(), (vm_offset_t)fp->f_fs, SBSIZE);
    (void)vm_deallocate(mach_task_self(), (vm_offset_t)fp->f_gd, fp->f_gd_size);
    fp->f_fs = 0;
  }
}
 ...
 * Open a file.
 */
int ext2_open_file(master_device_port, path, fp)
    ...
    *cp = c;
    ...
    if ((fp->i_ic.i_mode & IFMT) == IFLNK) {
...
#endif IC_FASTLINK
      {
        ...
        rc = device_read(fp->f_dev, 0, (recnum_t)fsbtodb(fs, disk_block),
                         (int)blksize(fs, fp, 0), (char **)&buf, &buf_size);
        if (rc)
          goto exit;
 ...
 */
void ext2_close_file(fp) register struct file *fp;
{
  register int i;
...
int ext2_file_is_directory(struct file *fp) {
  return (fp->i_ic.i_mode & IFMT) == IFDIR;
}

int ext2_file_is_regular(struct file *fp) {
 ...
 */
int ext2_read_file(fp, offset, start, size, resid)
register struct file *fp;
vm_offset_t offset;
vm_offset_t start;
vm_size_t size;
 ...
 */
int ext2_open_file_direct(dev, fdp, is_structured)
mach_port_t dev;
...
boolean_t is_structured;
{
  ...
  (void)vm_deallocate(mach_task_self(), (vm_offset_t)fs, SBSIZE);
  (void)vm_deallocate(mach_task_self(), (vm_offset_t)gd, gd_size);
 ...
 */
int ext2_add_file_direct(fdp, fp)
register struct file_direct *fdp;
register struct file *fp;
{
  register struct ext2_super_block *fs;
  ...
  if (!file_is_structured(fp)) {
    ...
    rc = device_get_status(fdp->fd_dev, DEV_GET_SIZE, result, &count);
    if (rc)
      return rc;
  ...
  */
  ...
  if (fdp->fd_blocks != 0) {
    ...
    (void)vm_deallocate(mach_task_self(), (vm_offset_t)fdp->fd_blocks,
...
int ext2_remove_file_direct(fdp)
 ...
 */
int ext2_page_read_file_direct(fdp, offset, size, addr, size_read)
register struct file_direct *fdp;
vm_offset_t offset;
vm_size_t size;
...
mach_msg_type_number_t *size_read; /* out */
{
  ...
  if (offset % DEV_BSIZE != 0 || size % DEV_BSIZE != 0)
  ...
  return (device_read(fdp->fd_dev, 0,
                      (recnum_t)(fdir_fsbtodb(fdp, disk_block) + btodb(off)),
                      (int)size, (char **)addr, size_read));
}
 ...
 */
int ext2_page_write_file_direct(fdp, offset, addr, size, size_written)
register struct file_direct *fdp;
vm_offset_t offset;
vm_offset_t addr;
vm_size_t size;
vm_offset_t *size_written; /* out */
{
   ...
   * stale data.
   */
  rc = device_write(fdp->fd_dev, 0,
                    (recnum_t)(fdir_fsbtodb(fdp, disk_block) + btodb(off)),
                    (char *)addr, size, &num_written);
