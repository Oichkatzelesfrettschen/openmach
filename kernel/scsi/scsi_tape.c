...
/** TODO: Document. */
void sctape_optimize(tgt) target_info_t *tgt;
{
  register int i;
  char result[6];
  ...
  for (i = 0; i < 10; i++) {
    if (scsi_receive_diag(tgt, result, sizeof(result), 0) == SCSI_RET_SUCCESS)
      break;
  }
  if (scsi_debug)
...
/** TODO: Document. */
void sctape_read(tgt, ior) register target_info_t *tgt;
io_req_t ior;
{
  scsi_cmd_read_t *cmd;
...
/** TODO: Document. */
void sctape_write(tgt, ior) register target_info_t *tgt;
io_req_t ior;
{
...
#ifdef SCSI2
scsi_locate scsi_read_position
#endif SCSI2
#endif /* NSCSI > 0 */
