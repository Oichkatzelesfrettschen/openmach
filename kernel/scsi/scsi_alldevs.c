 ...
 */
/** TODO: Document. */
void scsi_go_and_wait(tgt, insize, outsize, ior) target_info_t *tgt;
int insize, outsize;
io_req_t ior;
{
...
/** TODO: Document. */
void scsi_go(tgt, insize, outsize, cmd_only) target_info_t *tgt;
...
/** TODO: Document. */
int sizeof_scsi_command(unsigned char cmd) {
  switch ((cmd & SCSI_CODE_GROUP) >> 5) {
  case 0:
    return sizeof(scsi_command_group_0);
  case 1:
    return sizeof(scsi_command_group_1);
  case 2:
    return sizeof(scsi_command_group_2);
  /* 3,4 reserved */
  case 5:
    return sizeof(scsi_command_group_5);
  /* 6,7 vendor specific (!!) */
  case 6:
    return sizeof(scsi_command_group_2);
  }
...
/** TODO: Document. */
void scsi_print_inquiry(inq, pagecode, result) scsi2_inquiry_data_t *inq;
int pagecode;
char *result;
{
  ...
  static char *periph_state[4] = {"online", "offline", "?", "absent"};
  ...
  if (!result) {
    printf("\n\t%s%s %s (%s %x)", (inq->rmb) ? "" : "non-", "removable SCSI",
           (inq->periph_type > 10) ? "?device?"
                                   : periph_names[inq->periph_type],
           periph_state[inq->periph_qual & 0x3], inq->device_type);
    printf("\n\t%s%s%s", inq->iso ? "ISO-compliant, " : "",
           inq->ecma ? "ECMA-compliant, " : "",
           inq->ansi ? "ANSI-compliant, " : "");
    if (inq->ansi)
      printf("%s%d, ", "SCSI-", inq->ansi);
    if (inq->response_fmt == 2)
      printf("%s%s%s%s%s%s%s%s%s%s%s", "Supports: ", inq->aenc ? "AENC, " : "",
             inq->trmIOP ? "TrmIOP, " : "", inq->RelAdr ? "RelAdr, " : "",
  ...
  if (sns->error_class != SCSI_SNS_XTENDED_SENSE_DATA) {
    ...
    case SCSI_SNS_RECOVERED:
      scsi_error(tgt, SCSI_ERR_BAD | SCSI_ERR_SENSE, code,
                 sns->u.xtended.add_bytes);
      return TRUE;
      break;
    case SCSI_SNS_UNIT_ATN:
      scsi_error(tgt, SCSI_ERR_SENSE, code, sns->u.xtended.add_bytes);
      return TRUE;
      break;
...
#ifdef SCSI2
scsi_change_definition scsi_log_select scsi_log_sense scsi_long_mode_select
    scsi_read_buffer scsi_write_buffer
#endif SCSI2
...
static char *sns_msg[SCSI_SNS_RESERVED + 1] = {
    ...
    "Recovered",       "Unit not ready",
    "Medium",          "Hardware failure",
    "Illegal request", "Unit Attention Condition",
    "Protection",      "Blank Check",
    "Vendor Unique",   "Copy Operation Aborted",
    "Aborted Command", "Equal Comparison",
    "Volume Overflow", "Miscompare",
    "Reserved"};

void
    /** TODO: Document. */
    scsi_error(tgt, code, info, addtl) target_info_t *tgt;
unsigned code;
unsigned info;
char *addtl;
{
  ...
  if (SCSI_ERR_GRAVITY(code)) {
    printf("\n%s%c: %s %s %sx%x", "target ", unit, cmd, msg, "error, code ",
           info);
    if (addtl) {
      unsigned int add[3];
      bcopy(addtl, (char *)add, 3 * sizeof(int));
      printf("%s x%x x%x x%x", ", additional info ", add[0], add[1], add[2]);
    }
    printf("\n");
  }
}
...
/** TODO: Document. */
void scsi_print_sense_data(sns) scsi_sense_data_t *sns;
{
  printf("Sense data: %s%s, segment %d", sns_msg[sns->u.xtended.sense_key],
         " error", sns->u.xtended.segment_number);
  if (sns->u.xtended.ili)
    printf(", IncorrectLengthIndicator");
  ...
  if (sns->addr_valid) {
    unsigned int info;
    info = (sns->u.xtended.info0 << 24) | (sns->u.xtended.info1 << 16) |
           (sns->u.xtended.info2 << 8) | sns->u.xtended.info3;
...
/** TODO: Document. */
void scsi_print_add_sense_keys(key, qualif) register unsigned key, qualif;
{
