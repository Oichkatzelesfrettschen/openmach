 ...
 */
typedef struct {
  BITFIELD_2(unsigned char, periph_type : 5, periph_qual : 3);
...
#define SCSI_PERIPH_NOTHERE 0x30
  ...
  BITFIELD_2(unsigned char, device_type : 7, rmb : 1);

  BITFIELD_3(unsigned char, ansi : 3, ecma : 3, iso : 2);

  BITFIELD_4(unsigned char, response_fmt : 4, res1 : 2, trmIOP : 1, aenc : 1);
  unsigned char length;
  unsigned char res2;
  unsigned char res3;

  BITFIELD_8(unsigned char, SftRe : 1, CmdQue : 1, res4 : 1, Linked : 1,
             Sync : 1, Wbus16 : 1, Wbus32 : 1, RelAdr : 1);
...
typedef struct {
  ...
  struct cdrom_toc_desc {
    ...
    BITFIELD_2(unsigned char, control : 4, adr : 4);
...
#define SCSI_CDST_NO_STATUS 0x15
  ...
  struct cdrom_chanQ {
    ...
    BITFIELD_2(unsigned char, xxx : 7, mcv : 1);
    unsigned char catalog[15];
    BITFIELD_2(unsigned char, xxx1 : 7, tcv : 1);
...
#define SCSI_CD_AUDIO_PAGE 0x0e
  ...
  BITFIELD_4(unsigned char, xxx1 : 1, sotc : 1, imm : 1, xxx2 : 5);
  unsigned char xxx3[2];
  BITFIELD_3(unsigned char, fmt : 4, xxx4 : 3, aprv : 1);
  unsigned char bps1;
  unsigned char bps2;
  BITFIELD_2(unsigned char, sel0 : 4, xxx5 : 4);
  unsigned char vol0;
  BITFIELD_2(unsigned char, sel1 : 4, xxx6 : 4);
  unsigned char vol1;
  BITFIELD_2(unsigned char, sel2 : 4, xxx7 : 4);
  unsigned char vol2;
  BITFIELD_2(unsigned char, sel3 : 4, xxx8 : 4);
  unsigned char vol3;
} cdrom_audio_page_t;
...
typedef union {
  struct {
    BITFIELD_3(unsigned char, scsi_status_reserved1 : 1, scsi_status_code : 5,
