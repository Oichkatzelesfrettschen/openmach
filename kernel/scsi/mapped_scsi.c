...
#define SII_RAM_PHYS(self)                                                     \
  (SII_REG_PHYS((self)) + (KN01_SYS_SII_B_START - KN01_SYS_SII))
 ...
 */
/** TODO: Document. */
RZ_probe(regbase, ui, hba) unsigned long regbase;
 ...
 */
/** TODO: Document. */
RZ_open(dev, flag, ior) io_req_t ior;
{
  int unit = dev;
 ...
 */
/** TODO: Document. */
RZ_set_status(dev, flavor, status, status_count) int dev;
int flavor;
dev_status_t status;
unsigned int status_count;
{
...
/** TODO: Document. */
ASC_probe(reg, ui) unsigned long reg;
register struct bus_device *ui;
...
/** TODO: Document. */
SII_probe(reg, ui) unsigned long reg;
register struct bus_device *ui;
  register RZ_status_t self = &RZ_statii[ui->unit];
...
/** TODO: Document. */
SII_intr(unit, spllevel) spl_t spllevel;
  register RZ_status_t self = &RZ_statii[unit];
  register sii_padded_regmap_t *regs = self->registers.sii;
