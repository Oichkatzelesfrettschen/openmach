...
#define AHA_ADDRESS_GET(addr, val)                                             \
  {                                                                            \
    (val) = ((addr).msb << 16) | ((addr).mid << 8) | ((addr).lsb);             \
  }
...
struct aha_setup {
  BITFIELD_3(unsigned char, initiate_SDT : 1, enable_parity : 1, res : 6);
  ...
  struct {
    BITFIELD_3(unsigned char, offset : 4, period : 3, /* 200 + 50 * N */
               negotiated : 1);
  } SDT_params[8];
...
#define AHA_CCB_BDEV_RST 0x81
  BITFIELD_4(unsigned char, ccb_lun : 3, ccb_in : 1, ccb_out : 1,
             ccb_scsi_id : 3);
...
#define AHA_MB_GET_PTR(mbx, val) AHA_ADDRESS_GET((mbx)->mb.mb_ptr, (val))
  ...
  struct { /* ccb required In mbox */
    ...
    BITFIELD_4(unsigned char, mb_lun : 3, mb_isa_send : 1, mb_isa_recv : 1,
               mb_initiator_id : 3);
