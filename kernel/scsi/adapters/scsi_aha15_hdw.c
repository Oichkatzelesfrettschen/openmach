...
struct aha_softc {
  watchdog_t wd;
  decl_simple_lock_data(, aha_lock) unsigned int port; /* I/O port */
...
struct bus_driver aha_driver = {
    aha_probe, scsi_slave, scsi_attach, aha_go,    aha_std,
    "rz",      aha_dinfo,  "ahac",      aha_minfo, BUS_INTR_B4_PROBE};
...
/** TODO: Document. */
aha_target_state(tgt) target_info_t *tgt;
{
  if (tgt == 0)
 ...
 */
/** TODO: Document. */
aha_probe(port, ui) register port;
struct bus_ctlr *ui;
{
  int unit = ui->unit;
   ...
   */
  {
    ...
    if (!(st & AHA_CSR_INIT_REQ)) {
      outb(AHA_CONTROL_PORT(port), AHA_CTL_SOFT_RESET | AHA_CTL_HARD_RESET);
      while ((st = inb(AHA_STATUS_PORT(port))) & AHA_CSR_SELF_TEST)
        ;
    }
    if ((st & AHA_CSR_DATAO_FULL) || !(st & AHA_CSR_INIT_REQ))
      goto fail;
    ...
    st = inb(AHA_STATUS_PORT(port));
    if ((st & AHA_CSR_CMD_ERR) || ((st & AHA_CSR_DATAI_FULL) == 0))
      goto fail;
    ...
    { /* Check that port isn't 174x in enhanced mode
       ...
       */
      unsigned z;
      static unsigned port_table[] = {0,     0,     0x130, 0x134,
                                      0x230, 0x234, 0x330, 0x334};
      for (z = 0x1000; z <= 0xF000; z += 0x1000)
        if (inb(z + 0xC80) == 0x04 && inb(z + 0xC81) == 0x90 &&
   ...
   */
  {
    ...
    if (inq.board_id == 0x44 || inq.board_id == 0x45) {
      aha_command(port, AHA_EXT_BIOS, 0, 0, &extbios, sizeof(extbios), TRUE);
#ifdef AHADEBUG
       ...
       */
      extbios.flags = 0;
      aha_command(port, AHA_MBX_ENABLE, &extbios, sizeof(extbios), 0, 0, TRUE);
    }
  }
doconf:
  /*
   ...
   */
  {
    ...
    printf("%s%d: [dma ch %d intr ch %d] my SCSI id is %d", ui->name, unit, d,
           i, sc->initiator_id);
   ...
   */
  aha_command(port, AHA_CMD_FIND_DEVICES, 0, 0, &installed, sizeof(installed),
              TRUE);
...
/** TODO: Document. */
aha_init_1(aha) aha_softc_t aha;
{
  struct aha_init a;
  vm_offset_t phys;
 ...
 */
/** TODO: Document. */
aha_go(tgt, cmd_count, in_count, cmd_only) target_info_t *tgt;
boolean_t cmd_only;
{
  aha_softc_t aha;
  spl_t s;
  ...
  case 0xc9: /* vendor-spec: SCSI_CMD_DEC_PLAYBACK_CONTROL */
  {
    ...
    len = tgt->transient_state.out_count = cmd_count - cs;
...
/** TODO: Document. */
aha_prepare_rccb(tgt, rccb, virt, len) target_info_t *tgt;
struct aha_ccb_raw *rccb;
vm_offset_t virt;
vm_size_t len;
{
...
/** TODO: Document. */
aha_start_scsi(aha, ccb) aha_softc_t aha;
aha_ccb_t *ccb;
{
 ...
 */
/** TODO: Document. */
aha_target_intr(aha, mbi) aha_softc_t aha;
aha_mbox_t mbi;
{
  ...
  len = (mbi.mbt.mb_data_len_msb << 16) | (mbi.mbt.mb_data_len_mid << 8);
...

  {
    ...
    aha_prepare_rccb(initiator, rccb, (vm_offset_t)self->ior->io_data,
                     self->ior->io_count);
...
/** TODO: Document. */
aha_initiator_intr(aha, mbi) aha_softc_t aha;
aha_mbox_t mbi;
{
  struct aha_ccb_raw *rccb;
 ...
 */
/** TODO: Document. */
aha_bus_reset(aha) register aha_softc_t aha;
{
 ...
 */
/** TODO: Document. */
aha_reset_scsibus(aha) register aha_softc_t aha;
{
  register target_info_t *tgt;
  register port = aha->port;
  register int i;
 ...
 */
/** TODO: Document. */
aha_command(port, cmd, outp, outc, inp, inc,
            clear_interrupt) unsigned char *outp,
    *inp;
{
  register unsigned char st;
...
/** TODO: Document. */
aha_alloc_segment_list(tgt) target_info_t *tgt;
{
