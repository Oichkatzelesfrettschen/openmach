...
#define SII_COMMAND(regs, cs, ds, cmd)                                         \
  {                                                                            \
    (regs)->sii_cmd = ((cs) & 0x70) | ((ds) & 0x07) | (cmd);                   \
    wbflush();                                                                 \
  }
...
struct bus_driver sii_driver = {
    sii_probe, scsi_slave, scsi_attach, sii_go,    sii_std,
    "rz",      sii_dinfo,  "sii",       sii_minfo, /*BUS_INTR_B4_PROBE?*/};
...
struct script
    ...
    sii_script_cmd[] = {{SCSI_PHASE_CMD | SII_CON_CON, sii_script_true,
                         (SII_CMD_XFER | SII_CMD_DMA) | SII_CON_CON |
                             SCSI_PHASE_CMD},
                        {SCSI_PHASE_STATUS | SII_CON_CON, sii_get_status,
                         SII_CMD_XFER | SII_CON_CON | SCSI_PHASE_STATUS},
                        {SCSI_PHASE_MSG_IN | SII_CON_CON, sii_end_transaction,
                         SCRIPT_END}},
    ...
    sii_script_restart_cmd[] = {{SCSI_PHASE_STATUS | SII_CON_CON | SII_CON_DST,
                                 sii_get_status,
                                 SII_CMD_XFER | SII_CON_CON | SII_CON_DST |
                                     SCSI_PHASE_STATUS},
                                {SCSI_PHASE_MSG_IN | SII_CON_CON | SII_CON_DST,
                                 sii_end_transaction, SCRIPT_END}},

    /* Synchronous transfer negotiation */

    sii_script_try_synch[] = {{SCSI_PHASE_MSG_OUT | SII_CON_CON, sii_dosynch,
                               SCRIPT_END}},

    /* Disconnect sequence */

    sii_script_disconnect[] = {{SII_PHASE_DISC, sii_disconnected, SCRIPT_END}};
...
/** TODO: Document. */
sii_state(regs) sii_padded_regmap_t *regs;
{
  unsigned dmadr;
  ...
  db_printf("sc %x, dma %x @ x%X, cs %x, ds %x, cmd %x\n",
            (unsigned)regs->sii_sc1, (unsigned)regs->sii_dma_len, dmadr,
            (unsigned)regs->sii_conn_csr, (unsigned)regs->sii_data_csr,
            (unsigned)regs->sii_cmd);
}
/** TODO: Document. */
sii_target_state(tgt) target_info_t *tgt;
{
  if (tgt == 0)
...
/** TODO: Document. */
static bump(counter, value) register struct cnt *counter;
register unsigned int value;
{
  ...
  counter->usage = n;
  if (n == 0) {
    printf("{Counter at x%x overflowed with avg x%x}", counter, counter->avg);
    return;
  } else if (n == 1)
...
struct bus_ctlr *ui;
{
   ...
   */
  for (target_id = 0, dma_ptr = (char *)sii->buff; target_id < 8;
...
/** TODO: Document. */
sii_probe_timeout(tgt) target_info_t *tgt;
{
...
/** TODO: Document. */
static sii_wait(preg, until, complain) volatile unsigned short *preg;
{
  int timeo = 1000000;
...
/** TODO: Document. */
sii_reset(regs, quickly) register sii_padded_regmap_t *regs;
boolean_t quickly;
{
  int my_id;
 ...
 */
/** TODO: Document. */
sii_go(tgt, cmd_count, in_count, cmd_only) target_info_t *tgt;
boolean_t cmd_only;
{
  sii_softc_t sii;
  register spl_t s;
  ...
  if ((tgt->cur_cmd == SCSI_CMD_WRITE) ||
    ...
    copyout_gap16(ior->io_data, tgt->dma_ptr + (cmd_count << 1), len);
...
/** TODO: Document. */
sii_attempt_selection(sii) sii_softc_t sii;
{
  target_info_t *tgt;
  ...
  if (tgt->flags & TGT_DID_SYNCH) {
    if (tgt->transient_state.identify == 0xff)
      ...
      cmd = SII_CMD_SEL | SII_CMD_ATN | SII_CMD_CON | SII_CMD_XFER |
            SCSI_PHASE_MSG_OUT;
  ...
  if (regs->sii_sc1 & (SII_CS1_BSY | SII_CS1_SEL)) {
    ...
    regs->sii_csr =
        SII_CSR_IE | SII_CSR_PCE | SII_CSR_SLE | SII_CSR_HPM | SII_CSR_RSE;
    sii->script = 0;
    LOG(0x8c, 0);
  } else {
    regs->sii_cmd = cmd;
...
sii_intr(unit, spllevel) {
  ...
  if ((sii->state & SII_STATE_TARGET) || (cs & SII_CON_TGT)) {
...
/** TODO: Document. */
sii_target_intr(sii) register sii_softc_t sii;
{
...
register sii_softc_t sii;
{
  ...
  regs->sii_cmd =
      SII_CMD_XFER | SII_CON_CON | SCSI_PHASE_MSG_IN | (cs & SII_CON_DST);
  wbflush();
...
register sii_softc_t sii;
{
  ...
  if (sii->state & SII_STATE_COLLISION) {
    ...
    LOG(0xC, "dequeue");
    sii->next_target = (target_info_t *)dequeue_head(&sii->waiting_targets);
    sii_attempt_selection(sii);
  }
  return ret;
}
...
register sii_softc_t sii;
{
  ...
  if (len = sii->in_count) {
    if ((tgt->cur_cmd != SCSI_CMD_READ) &&
      ...
      careful_copyin_gap16(tgt, tgt->transient_state.dma_offset, len,
                           ds & SII_DTR_OBB, regs->sii_dma_1st_byte);
    }
    sii->in_count = 0;
  }
...
register sii_softc_t sii;
{
  ...
  regs->sii_cmd = SII_CMD_DMA | SII_CMD_XFER | SII_CMD_ATN | SII_CON_CON |
                  SCSI_PHASE_MSG_OUT;
  wbflush();
...
cmdp:
  ...
  sii->script =
      (tgt->cur_cmd == SCSI_CMD_INQUIRY) ? sii_script_data_in : sii_script_cmd;
  if (SCSI_PHASE(ds) == SCSI_PHASE_CMD)
    return TRUE;
 ...
 */
/** TODO: Document. */
sii_bus_reset(sii) register sii_softc_t sii;
{
  register sii_padded_regmap_t *regs = sii->regs;
...
register sii_softc_t sii;
{
  ...
  /* this is a band-aid */
  if ((SCSI_PHASE(cond) == SII_PHASE_DISC) && (cs & SII_CON_SCH)) {
    ds &= ~7;
    ds |= SII_PHASE_DISC;
 ...
 */
/** TODO: Document. */
sii_err_to_status(sii, cs, ds) register sii_softc_t sii;
{
...
register unsigned cs, ds;
{
  ...
  regs->sii_cmd = SII_CMD_DMA | SII_CMD_XFER | SCSI_PHASE_MSG_IN | SII_CON_CON |
                  (regs->sii_conn_csr & SII_CON_DST);
  wbflush();
  if (delayed_copy == 2)
 ...
 */
/** TODO: Document. */
sii_err_rdp(sii, cs, ds) register sii_softc_t sii;
{
  register sii_padded_regmap_t *regs;
  register target_info_t *tgt;
 ...
 */
/** TODO: Document. */
sii_err_phase_mismatch(sii, cs, ds) register sii_softc_t sii;
{
  register sii_padded_regmap_t *regs = sii->regs;
  register int match;
 ...
 */
/** TODO: Document. */
sii_reset_scsibus(sii) register sii_softc_t sii;
{
  ...
  if (tgt)
    log(LOG_KERN,
        ...
        tgt->transient_state.out_count, sii->in_count, sii->out_count,
...
/** TODO: Document. */
careful_copyin_gap16(tgt, offset, len, isaobb, obb) register target_info_t *tgt;
unsigned char obb;
{
...
/** TODO: Document. */
careful_copyout_gap16(tgt, offset, len) register target_info_t *tgt;
{
  register char *from, *to;
