...
typedef struct {
  ...
  /*#define		sci_irecv sci_iack	/* w:  Start dma receive,
   * initiator */
  /** TODO: Document. */
  PAD(pad7);
...
#define SCI_CLR_INTR(regs)                                                     \
  {                                                                            \
    register int temp = regs->sci_iack;                                        \
  }
...
/** TODO: Document. */
sci_buffer_init(dmar, ram) sci_dmaregs_t dmar;
volatile char *ram;
{
...
struct bus_driver sci_driver = {
    sci_probe, scsi_slave, scsi_attach, sci_go,    sci_std,
    "rz",      sci_dinfo,  "sci",       sci_minfo, BUS_INTR_B4_PROBE};

/*
 * Scripts
 */
struct script
    sci_script_data_in[] = {{SCSI_PHASE_DATAI | SCI_BUS_BSY, sci_dma_in},
                            {SCSI_PHASE_STATUS | SCI_BUS_BSY, sci_get_status},
                            {SCSI_PHASE_MSG_IN | SCI_BUS_BSY,
                             sci_end_transaction}},

    sci_script_data_out[] = {{SCSI_PHASE_DATAO | SCI_BUS_BSY, sci_dma_out},
                             {SCSI_PHASE_STATUS | SCI_BUS_BSY, sci_get_status},
                             {SCSI_PHASE_MSG_IN | SCI_BUS_BSY,
                              sci_end_transaction}},

    sci_script_cmd[] = {{SCSI_PHASE_STATUS | SCI_BUS_BSY, sci_get_status},
                        {SCSI_PHASE_MSG_IN | SCI_BUS_BSY, sci_end_transaction}},

    /* Synchronous transfer neg(oti)ation */

    sci_script_try_synch[] = {{SCSI_PHASE_MSG_OUT | SCI_BUS_BSY, sci_dosynch}},

    /* Disconnect sequence */

    sci_script_disconnect[] = {{SCI_PHASE_DISC, sci_disconnected}};
...
/** TODO: Document. */
sci_state(base) vm_offset_t base;
{
  ...
  for (i = 0; i < 2; i++) {
    regs = (sci_padded_regmap_t *)(base + (i ? STC_5380_B : STC_5380_A));
    dmar = (sci_dmaregs_t)((char *)regs + STC_DMAREG_OFF);
    SCI_DMADR_GET(dmar, dmadr);
    SCI_TC_GET(dmar, cnt);

    db_printf("scsi%d: ph %x (sb %x), mode %x, tph %x, csr %x, cmd %x, ", i,
              (unsigned)SCI_CUR_PHASE(regs->sci_bus_csr),
              (unsigned)regs->sci_bus_csr, (unsigned)regs->sci_mode,
              (unsigned)regs->sci_tcmd, (unsigned)regs->sci_csr,
...
/** TODO: Document. */
sci_target_state(tgt) target_info_t *tgt;
{
  if (tgt == 0)
...
/** TODO: Document. */
static LOG(e, f) char *f;
{
  sci_log[sci_logpt++] = (e);
  if (sci_logpt == LOGSIZE)
    sci_logpt = 0;
  if ((e) < MAXLOG_VALUE) {
    logtbl[(e)].name = (f);
    logtbl[(e)].count++;
...
struct bus_ctlr *ui;
{
  ...
  for (target_id = 0; target_id < 8; target_id++) {
    ...
    if (sci_select_target(regs, sc->initiator_id, target_id, FALSE) ==
...
/** TODO: Document. */
static sci_wait(preg, until) volatile unsigned char *preg;
{
  int timeo = 1000000;
  /* read it over to avoid bus glitches */
  while (((*preg & until) != until) || ((*preg & until) != until) ||
         ((*preg & until) != until)) {
    delay(1);
    if (!timeo--) {
...
/** TODO: Document. */
sci_data_out(regs, phase, count, data) register sci_padded_regmap_t *regs;
unsigned char *data;
{
...
loop:
  ...
  while ((regs->sci_bus_csr & SCI_BUS_REQ) &&
         (regs->sci_bus_csr & SCI_BUS_REQ) && (regs->sci_bus_csr & SCI_BUS_REQ))
    ;
  regs->sci_icmd = icmd;
  if (--count > 0)
    goto loop;
...
/** TODO: Document. */
sci_data_in(regs, phase, count, data) register sci_padded_regmap_t *regs;
unsigned char *data;
{
  register unsigned char icmd;
 ...
 */
/** TODO: Document. */
sci_go(tgt, cmd_count, in_count, cmd_only) target_info_t *tgt;
boolean_t cmd_only;
{
  sci_softc_t sci;
  register spl_t s;
...
/** TODO: Document. */
sci_attempt_selection(sci) sci_softc_t sci;
{
  target_info_t *tgt;
  ...
  if (tgt->flags & TGT_DID_SYNCH) {
    if (tgt->transient_state.identify == 0xff) {
      /* Select w/o ATN */
      ret =
          sci_select_target(regs, sci->sc->initiator_id, tgt->target_id, FALSE);
    } else {
      /* Select with ATN */
      ret =
          sci_select_target(regs, sci->sc->initiator_id, tgt->target_id, TRUE);
      if (ret == SCSI_RET_SUCCESS) {
        ...
        ok = (sci_data_out(regs, SCSI_PHASE_MSG_OUT, 1,
                           &tgt->transient_state.identify) == 0);
      }
    }
  } else if (tgt->flags & TGT_TRY_SYNCH) {
    /* Select with ATN, do the synch xfer neg */
    ret = sci_select_target(regs, sci->sc->initiator_id, tgt->target_id, TRUE);
    if (ret == SCSI_RET_SUCCESS) {
      while (SCI_CUR_PHASE(regs->sci_bus_csr) != SCSI_PHASE_MSG_OUT)
        ;
    ...
    ret = sci_select_target(regs, sci->sc->initiator_id, tgt->target_id, FALSE);
  }
...
sci_intr(unit) {
  ...
  if ((csr & SCI_CSR_DISC) && scp && (scp->condition == SCI_PHASE_DISC)) {
  ...
  if ((scp == 0) || (cmd & SCI_ICMD_LST) ||
      ((bs & (SCI_BUS_BSY | SCI_BUS_SEL | SCI_BUS_IO)) ==
       (SCI_BUS_SEL | SCI_BUS_IO))) {
...
/** TODO: Document. */
sci_target_intr(sci) register sci_softc_t sci;
{
...
register sci_softc_t sci;
{
  ...
  bs = regs->sci_bus_csr;
  reconn_pending = ((bs & (SCI_BUS_BSY | SCI_BUS_SEL | SCI_BUS_IO)) ==
                    (SCI_BUS_SEL | SCI_BUS_IO));
  if (!reconn_pending) {
    sci_release_bus(sci);
  } else {
...
register sci_softc_t sci;
{
  ...
  if (sci->state & SCI_STATE_COLLISION) {
    ...
    LOG(0xC, "dequeue");
    sci->next_target = (target_info_t *)dequeue_head(&sci->waiting_targets);
    sci_attempt_selection(sci);
  }
  return ret;
}
...
register sci_softc_t sci;
{
  ...
  if (len = sci->in_count) {
    ...
    if ((tgt->cur_cmd != SCSI_CMD_READ) &&
      ...
      sci_copyin(tgt, tgt->transient_state.dma_offset, len, 0, 0);
    }
  }
...
register sci_softc_t sci;
{
  ...
  if (sci->in_count == 0) {
    ...
    assert(xferred == 0);
    if (scsi_debug) {
      printf("{B %x %x %x (%x)}", sci->in_count, xferred, sci->extra_byte,
             extrab);
    }
    /* ++EXTRA_BYTE */
    ...
    SCI_CLR_INTR(regs);
    regs->sci_mode |=
        (advance_script ? SCI_MODE_DMA : (SCI_MODE_DMA | SCI_MODE_DMA_IE));
...
register sci_softc_t sci;
{
  ...
  if (sci->out_count == 0) {
    ...
    /* ship some more */
    dma_ptr = tgt->dma_ptr + tgt->transient_state.cmd_count +
...
quickie:
  ...
  dma_ptr = tgt->dma_ptr + tgt->transient_state.cmd_count +
            tgt->transient_state.dma_offset;
  tmp = (advance_script ? SCI_MODE_PAR_CHK | SCI_MODE_DMA | SCI_MODE_MONBSY
...
register sci_softc_t sci;
{
  ...
  if (!ok)
    printf("[tgt %d bad msg (%d): %x]", sci->active_target->target_id, len,
           *msgs);
...
#define scsi_period_to_sci(x) (x)
  ...
  sci_data_out(regs, SCSI_PHASE_MSG_OUT, sizeof(scsi_synch_xfer_req_t), p);
  icmd = regs->sci_icmd & ~(SCI_ICMD_DIFF | SCI_ICMD_TEST);
  icmd &= ~SCI_ICMD_ATN;
  regs->sci_icmd = icmd;
  sci_data_out(regs, SCSI_PHASE_MSG_OUT, 1, &p[sizeof(scsi_synch_xfer_req_t)]);
 ...
 */
/** TODO: Document. */
sci_bus_reset(sci) register sci_softc_t sci;
{
  register target_info_t *tgt;
  register sci_padded_regmap_t *regs = sci->regs;
  int i;
 ...
 */
/** TODO: Document. */
sci_err_to_status(sci, csr, bs) register sci_softc_t sci;
{
 ...
 */
/** TODO: Document. */
sci_reset_scsibus(sci) register sci_softc_t sci;
{
  ...
  if (tgt) {
    ...
    log(LOG_KERN,
        "Target %d was active, cmd x%x in x%x out x%x Sin x%x Sou x%x dmalen "
        "x%x\n",
        tgt->target_id, tgt->cur_cmd, tgt->transient_state.in_count,
        tgt->transient_state.out_count, sci->in_count, sci->out_count, cnt);
  }
...
/*static*/
/** TODO: Document. */
sci_copyin(tgt, offset, len, isaobb, obb) register target_info_t *tgt;
unsigned char obb;
{
...
/*static*/
/** TODO: Document. */
sci_copyout(tgt, offset, len) register target_info_t *tgt;
{
  register char *from, *to;
