...
struct bus_driver eaha_driver = {
    eaha_probe, scsi_slave, scsi_attach, eaha_go,    eaha_std,
    "rz",       eaha_dinfo, "eahac",     eaha_minfo, BUS_INTR_B4_PROBE};
...
#define EAHA_CMD_TARG_CMD 0x10
   ...
   */
  BITFIELD_3(unsigned char, cne : 1, xxx0 : 6, di : 1);
  BITFIELD_7(unsigned char, xxx1 : 2, ses : 1, xxx2 : 1, sg : 1, xxx3 : 1,
             dsb : 1, ars : 1);

  BITFIELD_5(unsigned char, lun : 3, tag : 1, tt : 2, nd : 1, xxx4 : 1);
  BITFIELD_7(unsigned char, dat : 1, dir : 1, st : 1, chk : 1, xxx5 : 2,
             rec : 1, nbr : 1);
...
typedef struct {
  watchdog_t wd;
  decl_simple_lock_data(, aha_lock) int port; /* I/O port */
...
/** TODO: Document. */
erccb *erccb_alloc(eaha_softc *eaha) {
  erccb *e;
  int x;

  do {
    while (eaha->toperccb == 0)
      ; /* Shouldn't be often or long, */
...
/** TODO: Document. */
void erccb_free(eaha_softc *eaha, erccb *e) {
  int x;
...
/** TODO: Document. */
void eaha_mboxout(int port, vm_offset_t phys) {
...
void eaha_command(/* start a command */
                  int port, erccb *_erccb) {
  int s;
  vm_offset_t phys = kvtophys((vm_offset_t)&_erccb->_eccb);
  while ((inb(G2STAT(port)) & 0x04) == 0)
    ; /*While MBO busy. TRM6-1 */
  s = splbio();
  eaha_mboxout(port, phys);
  while (inb(G2STAT(port)) & 1)
    ; /* While adapter busy. TRM6-2 */
...
/** TODO: Document. */
eaha_reset(eaha_softc_t eaha, boolean_t quick) {
  /*
...
/** TODO: Document. */
void eaha_init(eaha_softc_t eaha) { /* Do nothing - I guess */ }

/** TODO: Document. */
void eaha_bus_reset(eaha_softc_t eaha)
...
/** TODO: Document. */
void eaha_download(port, data, len) int port;
char *data;
unsigned len;
{
...
/** TODO: Document. */
void eaha_initscsi(data, len) char *data;
unsigned len;
{
...
/** TODO: Document. */
void eaha_noop() { /* Not implemented. TRM6-27 */ }
...
/** TODO: Document. */
void eaha_diagnostic(eaha) eaha_softc *eaha;
{
...
target_info_t *
/** TODO: Document. */
eaha_tgt_alloc(eaha_softc_t eaha, int id, target_info_t *tgt) {
  erccb *_erccb;
...
struct {
  ...
  unsigned char extra[254 - sizeof(scsi_sense_data_t)];
} eaha_xsns[NTARGETS]; /*must be bss to be contiguous*/

/* Enhanced adapter probe routine */

/** TODO: Document. */
eaha_probe(register int port, struct bus_ctlr *ui) {
  int unit = ui->unit;
  ...
  printf("%s%d: [port 0x%x intr ch %d] my SCSI id is %d", ui->name, unit, port,
         my_interrupt + 9, my_scsi_id);
...
#define MAX_ATTEMPTS 2
      ...
      do {
        ...
        eaha_go(&temp_targ, sizeof(scsi_cmd_test_unit_ready_t), 0, 0);
        ...
        if (temp_targ.done == SCSI_RET_NEED_SENSE) {
          ...
          if (eaha_retrieve_sense_info(eaha, temp_targ.target_id,
                                       temp_targ.lun) &&
              attempts == MAX_ATTEMPTS - 1) {

            printf("\nTarget %d Check Condition : ", temp_targ.target_id);
            scsi_print_sense_data(&eaha_xsns[temp_targ.target_id]);
            printf("\n");
          }
        }
      } while (temp_targ.done != SCSI_RET_SUCCESS &&
               temp_targ.done != SCSI_RET_ABORTED && ++attempts < MAX_ATTEMPTS);
...
/** TODO: Document. */
int eaha_retrieve_sense_info(eaha_softc_t eaha, int tid, int lun) {
  int result;
  int s;
  ...
  _erccb1->_eccb.command = EAHA_CMD_READ_SENS;
 ...
 */
/** TODO: Document. */
eaha_go(target_info_t *tgt, int cmd_count, int in_count,
        boolean_t cmd_only) /*lint: unused*/
{
  ...
  if (eaha->has_sense_info[tid]) {
    (void)eaha_retrieve_sense_info(eaha, tid, eaha->sense_info_lun[tid]);
  ...
  _erccb =
      (erccb *)((unsigned)tgt->cmd_ptr - (unsigned)&((erccb *)0)->_eccb.cdb);
  ...
  case SCSI_CMD_FORMAT_UNIT:
    ...
    len = tgt->transient_state.out_count =
        cmd_count - sizeof(scsi_command_group_0);
...
/** TODO: Document. */
eaha_prepare_rccb(target_info_t *tgt, erccb *_erccb, vm_offset_t virt,
                  vm_size_t len) {
...
/** TODO: Document. */
eaha_alloc_segment_list(target_info_t *tgt) {
 ...
 */
/** TODO: Document. */
eaha_reset_scsibus(register eaha_softc_t eaha) {
  ...
  for (i = 0; i < NECCBS; i++) {
    ...
    if (/*scsi_debug &&*/ tgt)
      printf("Target %d was active, cmd x%x in x%x out x%x\n", tgt->target_id,
             tgt->cur_cmd, tgt->transient_state.in_count,
...
boolean_t
/** TODO: Document. */
eaha_probe_target(target_info_t *tgt, io_req_t ior) {
  ...
  case 0x05: /* ccb complete with success after retry */
    ...
    _erccb =
        (erccb *)(((vm_offset_t)&eaha->I_hold_my_phys_address) +
                      (mbi - eaha->I_hold_my_phys_address) - (vm_offset_t) &
                  (((erccb *)0)->_eccb));
...
#ifdef NOTDEF
    if (!eaha_quiet && (!status->don || status->qf || status->sc ||
                        status->dover || status->ini || status->me)) {
 ...
 */
/** TODO: Document. */
eaha_target_intr(eaha_softc_t eaha, unsigned int mbi, unsigned int peer) {
...

  {
    ...
    eaha_prepare_rccb(initiator, _erccb, (vm_offset_t)self->ior->io_data,
                      self->ior->io_count);
...
/** TODO: Document. */
eaha_initiator_intr(eaha_softc_t eaha, erccb *_erccb) {
  ...
  eaha->has_sense_info[tgt->target_id] = (tgt->done == SCSI_RET_NEED_SENSE);
