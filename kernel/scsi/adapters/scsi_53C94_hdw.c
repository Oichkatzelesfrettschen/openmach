...
#define ANY 0xff
#define SCRIPT_MATCH(csr, ir, value)                                           \
  ((SCSI_PHASE(csr) == (value)) ||                                             \
   (((value) == ANY) && ((ir) & (ASC_INT_DISC | ASC_INT_FC))))
...
struct bus_driver asc_driver = {
    asc_probe, scsi_slave, scsi_attach, asc_go,    asc_std,
    "rz",      asc_dinfo,  "asc",       asc_minfo, BUS_INTR_B4_PROBE};

int asc_clock_speed_in_mhz[NASC] = {25, 25, 25, 25}; /* original 3max */

/** TODO: Document. */
asc_set_dmaops(unit, dmaops) unsigned int unit;
...
struct script
    ...
    asc_script_try_synch[] = {{SCSI_PHASE_MSG_OUT, ASC_CMD_I_COMPLETE, 0,
                               asc_dosynch},
                              {SCSI_PHASE_MSG_IN, ASC_CMD_MSG_ACPT, 0,
                               asc_get_status},
                              {ANY, SCRIPT_END, 0, asc_end}},

    asc_script_simple_cmd[] = {{SCSI_PHASE_STATUS, ASC_CMD_I_COMPLETE, 0,
                                asc_clean_fifo},
                               {SCSI_PHASE_MSG_IN, ASC_CMD_MSG_ACPT, 0,
                                asc_get_status},
                               {ANY, SCRIPT_END, 0, asc_end}},

    asc_script_disconnect[] = {{ANY, ASC_CMD_ENABLE_SEL, 0, asc_disconnected},
                               /**/ {SCSI_PHASE_MSG_IN, ASC_CMD_MSG_ACPT, 0,
                                     asc_reconnect}},

    asc_script_restart_data_in[] =
        {/* starts after disconnect */
         {SCSI_PHASE_DATAI, ASC_CMD_XFER_INFO | ASC_CMD_DMA, 0, asc_dma_in_r},
         {SCSI_PHASE_STATUS, ASC_CMD_I_COMPLETE, 0, asc_clean_fifo},
         {SCSI_PHASE_MSG_IN, ASC_CMD_MSG_ACPT, 0, asc_get_status},
         {ANY, SCRIPT_END, 0, asc_end}},

    asc_script_restart_data_out[] =
        {/* starts after disconnect */
     ...
     * that disconnects
     */
    asc_script_data_in_wd[] =
        {/* started with SEL & DMA & allow disconnect */
     ...
     * Target mode scripts
     */
    asc_script_t_data_in[] = {{SCSI_PHASE_CMD, ASC_CMD_RCV_DATA | ASC_CMD_DMA,
                               0, asc_dma_in_r},
                              {SCSI_PHASE_DATAO, ASC_CMD_TERM, 0,
                               asc_put_status},
                              {ANY, SCRIPT_END, 0, asc_end}},
...
/** TODO: Document. */
asc_state(regs) asc_padded_regmap_t *regs;
{
...
/** TODO: Document. */
asc_target_state(tgt) target_info_t *tgt;
{
  if (tgt == 0)
  ...
  if (tgt == 0)
    return 0;
  db_printf(
      "@x%x: fl %x dma %X+%x cmd %x@%X id %x per %x off %x ior %X ret %X\n",
      tgt, tgt->flags, tgt->dma_ptr, tgt->transient_state.dma_offset,
      tgt->cur_cmd, tgt->cmd_ptr, (long)tgt->target_id, (long)tgt->sync_period,
      (long)tgt->sync_offset, tgt->ior, (long)tgt->done);
 ...
 */
/** TODO: Document. */
asc_probe(reg, ui) vm_offset_t reg;
struct bus_ctlr *ui;
{
  int unit = ui->unit;
...
/** TODO: Document. */
asc_reset(asc, quick, special_dma) asc_softc_t asc;
{
  char my_id;
  int ccf;
  asc_padded_regmap_t *regs;
 ...
 */
/** TODO: Document. */
asc_go(tgt, cmd_count, in_count, cmd_only) target_info_t *tgt;
boolean_t cmd_only;
{
  asc_softc_t asc;
  register spl_t s;
  ...
  case 0xc9: /* vendor-spec: SCSI_CMD_DEC_PLAYBACK_CONTROL */
    ...
    tgt->transient_state.out_count = cmd_count - tgt->transient_state.cmd_count;
    scp = asc_script_data_out;
    LOG(0x1c, "cmdop");
    LOG(0x80 + tgt->cur_cmd, 0);
    break;
...
/** TODO: Document. */
asc_attempt_selection(asc) asc_softc_t asc;
{
  target_info_t *tgt;
  asc_padded_regmap_t *regs;
 ...
 */
/** TODO: Document. */
asc_intr(unit, spllevel) spl_t spllevel;
{
  ...
  if ((asc->state & ASC_STATE_TARGET) || (ir & (ASC_INT_SEL | ASC_INT_SEL_ATN)))
...
/** TODO: Document. */
asc_target_intr(asc, csr, ir) register asc_softc_t asc;
...
register asc_softc_t asc;
{
  ...
  asc->state &= ~ASC_STATE_TARGET;
  asc->regs->asc_syn_p = 0;
  mb();
  asc->regs->asc_syn_o = 0;
  mb();
...
register asc_softc_t asc;
{
  ...
  if (asc->state & ASC_STATE_COLLISION) {
    ...
    LOG(0xC, "dequeue");
    asc->next_target = (target_info_t *)dequeue_head(&asc->waiting_targets);
    asc_attempt_selection(asc);
  }
  return ret;
}
...
register asc_softc_t asc;
{
  ...
  regs->asc_fifo = 0;
  mb();
  regs->asc_fifo = SCSI_COMMAND_COMPLETE;
  mb();
...
/** TODO: Document. */
asc_dma_in_r(asc, csr, ir) register asc_softc_t asc;
{
  register target_info_t *tgt;
  register asc_padded_regmap_t *regs = asc->regs;
  register int count;
  ...
  if (asc->in_count == 0) {
    ...
    count = (*asc->dma_ops->restart_datain_1)(asc->dma_state, tgt);
     ...
     */
    count = (*asc->dma_ops->restart_datain_2)(asc->dma_state, tgt, xferred);
    ...
    (*asc->dma_ops->restart_datain_3)(asc->dma_state, tgt);
...
register asc_softc_t asc;
{
  ...
  if ((*asc->dma_ops->start_dataout)(asc->dma_state, tgt,
                                     (volatile unsigned *)&regs->asc_cmd,
...
register asc_softc_t asc;
{
  ...
  if (asc->out_count == 0) {
     ...
     */
    count = (*asc->dma_ops->restart_dataout_1)(asc->dma_state, tgt);
    ...
    count = (*asc->dma_ops->restart_dataout_2)(asc->dma_state, tgt, xferred);
    ...
    asc->extra_count = (*asc->dma_ops->restart_dataout_3)(
        asc->dma_state, tgt, (volatile unsigned *)&regs->asc_fifo);
    ASC_TC_PUT(regs, count);
    readback(regs->asc_cmd);
    regs->asc_cmd = asc->script->command;
    mb();
...
register unsigned char csr, ir;
{
  ...
  mb();
  regs->asc_fifo = asc_to_scsi_period(asc_min_period, asc->clk);
  mb();
  regs->asc_fifo = offs;
  mb();
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (i == 1)
      tgt->sync_period = scsi_period_to_asc(c, asc->clk);
    if (i == 2)
      tgt->sync_offset = c;
  }
...
cmd:
  ...
  if (SCSI_PHASE(csr) == SCSI_PHASE_CMD) {
    ...
    mb();
    regs->asc_cmd = ASC_CMD_XFER_PAD; /*0x18*/
    mb();
...
/** TODO: Document. */
asc_odsynch(asc, initiator) register asc_softc_t asc;
...
bad:
  ...
  regs->asc_fifo = SCSI_MESSAGE_REJECT;
  mb();
  regs->asc_cmd = ASC_CMD_SND_MSG;
  readback(regs->asc_cmd);
  asc_wait(regs, ASC_CSR_INT, 1);
 ...
 */
/** TODO: Document. */
asc_bus_reset(asc) register asc_softc_t asc;
{
  register asc_padded_regmap_t *regs = asc->regs;
...

{
  ...
  if ((msgs[0] == SCSI_DISCONNECT) ||         /* completed */
    ...
    printf("[tgt %d bad SDP: x%x]", tgt->target_id, *((unsigned short *)msgs));
...

{
  ...
  /* Get IDENTIFY message */
  {
    ...
    if (i) {
      target_info_t *tgt1;
      for (tgt1 = tgt->next_lun; tgt1 && tgt1 != tgt; tgt1 = tgt1->next_lun)
        if (tgt1->lun == i) {
          tgt = tgt1;
          break;
        }
...
register unsigned csr, ir;
{
  ...
  if (ir & ASC_INT_SEL_ATN) {
    ...
    csr = get_reg(regs, asc_csr);
    mb();
    if ((SCSI_PHASE(csr) == SCSI_PHASE_MSG_OUT) && fifo_count(regs))
  ...
  case 1:
  case 2:
    len = get_fifo(regs); /* xxx lba1 */
    mb();
    len = get_fifo(regs); /* xxx lba2 */
    mb();
    len = get_fifo(regs); /* xxx lba3 */
    mb();
    len = get_fifo(regs); /* xxx lba4 */
    mb();
    len = get_fifo(regs); /* xxx xxx */
    mb();
    len = get_fifo(regs) << 8;
    mb();
    len |= get_fifo(regs);
...

  {
    ...
    case SCSI_CMD_TEST_UNIT_READY:
      scp = asc_script_t_data_in + 1;
      break;
    case SCSI_CMD_SEND:
      scp = asc_script_t_data_in;
      break;
    default:
      scp = asc_script_t_data_out;
      break;
    }
...
private void
/** TODO: Document. */
pick_up_oddb(target_info_t *tgt) {
  register char *lastp;
 ...
 */
/** TODO: Document. */
asc_err_generic(asc, csr, ir) register asc_softc_t asc;
{
  LOG(0x13, "err_generic");

  /* handle non-existant or powered off devices here */
  if ((ir == ASC_INT_DISC) && (asc_isa_select(asc->cmd_was)) &&
      (ASC_SS(asc->ss_was) == 0)) {
    /* Powered off ? */
  ...
  case SCSI_PHASE_CMD:
    ...
    if (asc->script == asc_script_try_synch) {
      ...
      regs->asc_cmd = ASC_CMD_CLR_ATN;
      readback(regs->asc_cmd);
      asc->regs->asc_cmd = ASC_CMD_XFER_PAD | ASC_CMD_DMA; /*0x98*/
      mb();
      printf(" did not like SYNCH xfer ");
      return;
    }
 ...
 */
/** TODO: Document. */
asc_err_to_status(asc, csr, ir) register asc_softc_t asc;
{
 ...
 */
/** TODO: Document. */
asc_err_disconn(asc, csr, ir) register asc_softc_t asc;
register unsigned char csr, ir;
{
  register asc_padded_regmap_t *regs;
  register target_info_t *tgt;
  ...
  case SCSI_PHASE_DATAO:
    ...
    if (asc->out_count) {
      ...
      callback = (*asc->dma_ops->disconn_1)(asc->dma_state, tgt, xferred);
       ...
       */
      callback = (*asc->dma_ops->disconn_2)(asc->dma_state, tgt);
    }
    asc->extra_count = 0;
  ...
  case SCSI_PHASE_DATAI:
    ...
    if (asc->in_count) {
      ...
      callback = (*asc->dma_ops->disconn_3)(asc->dma_state, tgt, xferred);
  ...
  case SCSI_PHASE_STATUS:
    ...
    if (asc->state & ASC_STATE_DMA_IN) {
      ...
      if (asc->in_count) {
        ...
        callback = (*asc->dma_ops->disconn_4)(asc->dma_state, tgt, xferred);
      }
      /*
       * Handle obb if we have to.   DMA code has
       * updated pointers and flushed buffers.
      ...
      if ((count == 0) && (tgt->transient_state.out_count == asc->out_count)) {
        ...
        callback = (*asc->dma_ops->disconn_5)(asc->dma_state, tgt, xferred);
 ...
 */
/** TODO: Document. */
asc_reset_scsibus(asc) register asc_softc_t asc;
{
  register target_info_t *tgt = asc->active_target;
  register asc_padded_regmap_t *regs = asc->regs;
  register int ir;
  ...
  if (scsi_debug && tgt) {
    ...
    mb();
    printf("Target %d was active, cmd x%x in x%x out x%x Sin x%x Sou x%x "
           "dmalen x%x\n",
           tgt->target_id, tgt->cur_cmd, tgt->transient_state.in_count,
           tgt->transient_state.out_count, asc->in_count, asc->out_count,
           dmalen);
  }
  ir = get_reg(regs, asc_intr);
  mb();
  if ((ir & ASC_INT_RESEL) &&
      (SCSI_PHASE(regs->asc_csr) == SCSI_PHASE_MSG_IN)) {
