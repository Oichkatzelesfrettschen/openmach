...
/* our own minphys */
/** TODO: Document. */
void rz_minphys(ior) io_req_t ior;
{
#ifdef MACH_KERNEL
#else  /*MACH_KERNEL*/
...
#else  /*MACH_KERNEL*/
  return physio(tgt->dev_ops->strategy, getbp(dev), dev, IO_READ, rz_minphys,
                ior);
#endif /*MACH_KERNEL*/
}
...
io_req_t ior;
{
  ...
  return block_io(tgt->dev_ops->strategy, rz_minphys, ior);
#else  /*MACH_KERNEL*/
  return physio(tgt->dev_ops->strategy, getbp(dev), dev, IO_WRITE, rz_minphys,
                ior);
#endif /*MACH_KERNEL*/
}
...
char *info;
{
  ...
  case D_INFO_BLOCK_SIZE:
    *((int *)info) =
        scsi_softc[rzcontroller(dev)]->target[rzslave(dev)]->block_size;
    break;
  default:
...
void
    /** TODO: Document. */
    rz_simpleq_strategy(ior, start) io_req_t ior;
void (*start)();
{
  target_info_t *tgt;
...
/** TODO: Document. */
tz_ioctl(dev, cmd, data, flag) {
  return rz_ioctl(TAPE_UNIT(dev), cmd, data, flag);
}
