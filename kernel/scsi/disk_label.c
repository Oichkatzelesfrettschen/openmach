 ...
 */
struct disklabel default_label = {DISKMAGIC,
                                  DTYPE_SCSI,
                                  0,
                                  "SCSI",
                                  "",
...
/** TODO: Document. */
fudge_bsd_label(struct disklabel *label, int type, int total_secs, int heads,
                int sectors, int sectorsize, int n) {
  *label = default_label;
  ...
  if (slice == 0) /* compatability slice */
  {
    ...
    if (array[0].type == DISKPART_DOS) {
      ...
      for (i = 0; i < array[0].nsubs; i++) {
        s = &array[0].subs[i];
        if (s->type == DISKPART_BSD || s->type == DISKPART_VTOC) {
          if (part > s->nsubs)
            return 0;
    ...
    if (array[0].type != DISKPART_DOS || slice > array[0].nsubs)
      return 0;
...
static inline void fill_array(struct diskpart *array, int start, int size,
                              struct diskpart *subs, int nsubs, short type,
                              short fsys) {
  array->start = start;
  array->size = size;
  array->subs = subs;
...
void print_array(struct diskpart *array, int level) {
  ...
  for (i = 0; i < array[0].nsubs; i++) {
    ...
    printf("%c: %d, %d, %d, %d (%d subparts)\n", 'a' + i, subs[i].start,
           subs[i].size, subs[i].fsys, subs[i].type, subs[i].nsubs);
    if (subs[i].nsubs > 0)
...
/** TODO: Document. */
int get_dos(struct diskpart *array, char *buff, int start, void *driver_info,
            int (*bottom_read_fun)(), char *name, int max_part) {
  ...
  for (i = 0, pp = (struct bios_partition_info *)mp->partitions; i < count;
       i++, pp++) {

    fill_array(&array[i], pp->offset, pp->n_sectors, NULL, 0, DISKPART_NONE,
               pp->systid);
  ...
  while (ext >= 0) {
    ...
    ext = -1;
    for (j = 0, pp = (struct bios_partition_info *)mp->partitions; j < 4;
         j++, pp++) {

      if (pp->systid && (pp->systid != DOS_EXTENDED)) {
        if (count < max_part) {
          fill_array(&array[count], mybase + pp->offset, pp->n_sectors, NULL, 0,
...
int get_disklabel(struct diskpart *array, char *buff, int start,
                  void *driver_info, int (*bottom_read_fun)(), char *name,
                  int max_part) {
  ...
  for (i = 0; i < count; i++)
    fill_array(&array[i], /* mybase + */
               dlp->d_partitions[i].p_offset, dlp->d_partitions[i].p_size, NULL,
               0, DISKPART_NONE, dlp->d_partitions[i].p_fstype);
...
/* VTOC in sector 29 */
/** TODO: Document. */
int get_vtoc(struct diskpart *array, char *buff, int start, void *driver_info,
             int (*bottom_read_fun)(), char *name, int max_part) {
  struct evtoc *evp;
  int n, i;
  ...
  for (i = 0; i < n; i++)
    fill_array(&array[i], /* mybase + */
               evp->part[i].p_start, evp->part[i].p_size, NULL, 0,
               DISKPART_NONE, FS_BSDFFS);
...
/* NOT TESTED! */
/** TODO: Document. */
int get_omron(struct diskpart *array, char *buff, int start, void *driver_info,
              int (*bottom_read_fun)(), char *name, int max_part) {
  ...
  (*bottom_read_fun)(driver_info, start + OMRON_LABEL_BYTE_OFFSET / SECTOR_SIZE,
                     buff);
...
#else
    label->d_secperunit =
        label->d_ncylinders * label->d_ntracks * label->d_nsectors;
#endif 0
...
/* NOT TESTED! */
/** TODO: Document. */
int get_dec(struct diskpart *array, char *buff, int start, void *driver_info,
            int (*bottom_read_fun)(), char *name, int max_part) {
  struct disklabel *label;
  ...
  (*bottom_read_fun)(driver_info, start + DEC_LABEL_BYTE_OFFSET / SECTOR_SIZE,
                     buff);
...
int get_only_partition(void *driver_info, int (*bottom_read_fun)(),
                       struct diskpart *array, int array_size, int disk_size,
                       char *drive_name) {
  ...
  n = get_dos(&array[1], buff, 0, driver_info, (bottom_read_fun), drive_name,
              arrsize);

  if (n > 0) {
    fill_array(array, 0, disk_size, &array[1], n, DISKPART_DOS,
               256 + DISKPART_DOS);
    arrsize -= n;
...
#endif
        cnt =
            get_disklabel(&array[n + 1], buff, array[i + 1].start, driver_info,
                          (bottom_read_fun), drive_name, arrsize);

        if (cnt > 0) {
          arrsize -= cnt;
          fill_array(&array[i + 1], array[i + 1].start, array[i + 1].size,
                     &array[n + 1], cnt, DISKPART_BSD, array[i + 1].fsys);
        }
        n += cnt;
      }
...
#endif
        cnt = get_vtoc(&array[n + 1], buff, array[i + 1].start, driver_info,
                       (bottom_read_fun), drive_name, arrsize);

        if (cnt > 0) {
          arrsize -= cnt;
          fill_array(&array[i + 1], array[i + 1].start, array[i + 1].size,
                     &array[n + 1], cnt, DISKPART_VTOC, array[i + 1].fsys);
        }
        n += cnt;
      }
  }
  ...
  if (n == 0) {
    fill_array(array, 0, disk_size, &array[1], n, DISKPART_BSD,
               256 + DISKPART_BSD);
    n = get_disklabel(&array[1], buff, 0, driver_info, (bottom_read_fun),
                      drive_name, arrsize);
  }
  ...
  if (n == 0) {
    fill_array(array, 0, disk_size, &array[1], n, DISKPART_VTOC,
               256 + DISKPART_VTOC);
    n = get_vtoc(&array[1], buff, 0, driver_info, (bottom_read_fun), drive_name,
                 arrsize);
  }
#if 0
