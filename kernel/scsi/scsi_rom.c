...
/** TODO: Document. */
char *sccdrom_name(boolean_t internal) { return internal ? "rz" : "CD-ROM"; }

/** TODO: Document. */
int scsi_pause_resume(target_info_t *tgt, boolean_t stop_it, io_req_t ior) {
...
/** TODO: Document. */
scsi_play_audio(target_info_t *tgt, unsigned int start, unsigned int len,
                boolean_t relative_address, io_req_t ior) {
...
/** TODO: Document. */
scsi_play_audio_long(target_info_t *tgt, unsigned int start, unsigned int len,
                     boolean_t relative_address, io_req_t ior) {
...
/** TODO: Document. */
scsi_play_audio_msf(target_info_t *tgt, int sm, int ss, int sf, int em, int es,
                    int ef, io_req_t ior) {
...
/** TODO: Document. */
scsi_play_audio_track_index(target_info_t *tgt, int st, int si, int et, int ei,
                            io_req_t ior) {
...
/** TODO: Document. */
scsi_play_audio_track_relative(target_info_t *tgt, unsigned int lba, int st,
                               unsigned int len, io_req_t ior) {
...
/** TODO: Document. */
scsi_play_audio_track_relative_long(target_info_t *tgt, unsigned int lba,
                                    int st, unsigned int len, io_req_t ior) {
...
/** TODO: Document. */
scsi_read_header(target_info_t *tgt, boolean_t msf_format, unsigned int lba,
                 unsigned int allocsize, io_req_t ior) {
...
/** TODO: Document. */
scsi_read_subchannel(target_info_t *tgt, boolean_t msf_format,
                     unsigned int data_format, unsigned int trackno,
                     io_req_t ior) {
  ...
  case SCSI_CMD_RS_FMT_ISRC:
    allocsize = sizeof(cdrom_chan_isrc_t);
    break;
  }
...
/** TODO: Document. */
scsi_read_toc(target_info_t *tgt, boolean_t msf_format, int trackno,
              int allocsize, io_req_t ior) {
...
/** TODO: Document. */
scsi2_mode_select(target_info_t *tgt, boolean_t save, unsigned char *page,
                  int pagesize, io_req_t ior) {
 ...
 */
/** TODO: Document. */
cdrom_vendor_specific(target_info_t *tgt, scsi_command_group_2 *cmd,
                      unsigned char *params, int paramlen, int retlen,
                      io_req_t ior) {
