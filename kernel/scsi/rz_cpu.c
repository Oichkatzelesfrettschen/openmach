...
/** TODO: Document. */
void sccpu_new_initiator(self, initiator) target_info_t *self, *initiator;
{
  initiator->dev_ops = scsi_cpu_protocol;
  if (initiator == self) {
    self->flags =
        TGT_DID_SYNCH | TGT_FULLY_PROBED | TGT_ONLINE | TGT_ALIVE | TGT_US;
...
/** TODO: Document. */
void sccpu_strategy(ior) register io_req_t ior;
{
  void sccpu_start();
...
/** TODO: Document. */
void sccpu_start(tgt, done) target_info_t *tgt;
boolean_t done;
{
  io_req_t head, ior;
  scsi_ret_t ret;
...
/** TODO: Document. */
mread_test(max) register int max;
{
  register int i;
...
/** TODO: Document. */
mwrite_test(max) register int max;
{
  register int i;
  register volatile int *addr = mem_read_address;
...
/** TODO: Document. */
mcopy_test(max) register int max;
{
