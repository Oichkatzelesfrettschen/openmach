...
/** TODO: Document. */
scdisk_strategy(),
    sctape_strategy(), sccpu_strategy(),
    /** TODO: Document. */
    sccomm_strategy();
extern void
/** TODO: Document. */
...
/** TODO: Document. */
scdisk_start(),
    sctape_start(), sccpu_start(), sccomm_start();
...
scsi_devsw_t scsi_devsw[] = {
    ...
    /* SCSI_DISK */ {scdisk_name, SCSI_OPTIMIZE_NULL, scdisk_open,
                     SCSI_CLOSE_NULL, scdisk_strategy, scdisk_start,
                     scdisk_get_status, scdisk_set_status},

    /* SCSI_TAPE */
    {sctape_name, sctape_optimize, sctape_open, sctape_close, sctape_strategy,
     sctape_start, sctape_get_status, sctape_set_status},
    ...
    /* SCSI_WORM */
    {scworm_name, SCSI_OPTIMIZE_NULL, scdisk_open, SCSI_CLOSE_NULL,
     scdisk_strategy, scdisk_start, scdisk_get_status, scdisk_set_status},

    /* SCSI_CDROM */
    {sccdrom_name, SCSI_OPTIMIZE_NULL, scdisk_open, SCSI_CLOSE_NULL,
     scdisk_strategy, scdisk_start, scdisk_get_status, scdisk_set_status},
    /* scsi2 */
    /* SCSI_SCANNER */ {scscn_name, SCSI_OPTIMIZE_NULL, /*XXX*/},

    /* SCSI_MEMORY */
    {scmem_name, SCSI_OPTIMIZE_NULL, scdisk_open, SCSI_CLOSE_NULL,
     scdisk_strategy, scdisk_start, scdisk_get_status, scdisk_set_status},

    /* SCSI_J_BOX */ {scjb_name, SCSI_OPTIMIZE_NULL, /*XXX*/},

    /* SCSI_COMM */
    {sccomm_name, SCSI_OPTIMIZE_NULL,
#if (NCENDATA > 0)
     sccomm_open, sccomm_close, sccomm_strategy, sccomm_start,
...
unsigned reg;
{
  ...
  if (tgt->dev_ops)
	...
	tgt->unit_no = ui->slave;	/* incorrect, but needed early */
        ...
        if (BGET(scsi_no_synchronous_xfer, (unsigned char)sc->masterno,
                 tgt->target_id))
         ...
         */
        scsi_print_inquiry((scsi2_inquiry_data_t *)inq, SCSI_INQ_STD_DATA,
                           tgt->tgt_name);
        if (scsi_debug)
          scsi_print_inquiry((scsi2_inquiry_data_t *)inq, SCSI_INQ_STD_DATA, 0);
...
/** TODO: Document. */
scsi_verify_state(...){
  verify_state : send test_unit_ready up to 3 times,
  each time it fails(with check condition)
      send a requeste_sense.It is ok to get UNIT ATTENTION the first time only,
  NOT READY the second,
  only GOOD the last time.If you get BUSY or RESERVATION CONFLICT retry.
}
...
initialize:
  send start_unit with immed = 0(->disconnect),
                       if fails with check condition send requeste_sense
                           and if "illegal request" proceed anyways
                               .Retry on BUSY.Do a verify_state,
                       then disks
      : -mode_sense(current) if ANSI2 or needed by vendor(!!!!) and if check -
        condition & illegal - request goto capacity - mode_sense(changeable) -
 ...
 */
/** TODO: Document. */
void scsi_attach(ui) register struct bus_device *ui;
{
...
io_req_t ior;
{
...

  {
    ...
    mi.unit = sc->masterno;
    ui.mi = &mi;
    printf("%s at slave %d ", (*tgt->dev_ops->driver_name)(TRUE), target_id);
    scsi_attach(&ui);
  }
 ...
 */
/** TODO: Document. */
void scsi_watchdog(hw) watchdog_t *hw;
{
  spl_t s = splbio();
 ...
 */
/** TODO: Document. */
void scsi_bus_was_reset(sc) scsi_softc_t *sc;
{
