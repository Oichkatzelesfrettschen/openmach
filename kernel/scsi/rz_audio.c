...
private scsi_devsw_t scsi_audio = {sccdrom_name, 0,        0, 0,
                                   cd_strategy,  cd_start, 0, 0};
...
#define if_it_can_do(some_cmd)                                                 \
  if (tgt->dev_info.cdrom.violates_standards &&                                \
    ...
    rc = (*tgt->dev_info.cdrom.violates_standards->some_cmd)(tgt, cmd, ior);   \
  else
...
io_return_t
/** TODO: Document. */
cd_open(int dev, dev_mode_t mode, io_req_t req) {
  scsi_softc_t *sc = 0;
  target_info_t *tgt;
  int ret;
  scsi_ret_t rc;
  ...
  {
    char *cmd = 0;
    if_it_can_do(can_play_audio) rc = scsi_play_audio(tgt, 0, 0, FALSE, ior);
  }
...
bad:
  if (ior)
    io_req_free(ior);
  if (mem)
    kmem_free(kernel_map, mem, PAGE_SIZE);
...
cd_read(int dev, io_req_t ior) {
  ...
  if (scsi_debug)
    printf("Got read req for %d bytes\n", count);
...
io_return_t
/** TODO: Document. */
cd_command(int dev, char *cmd, int count, io_req_t req) {
  target_info_t *tgt;
  io_req_t ior;
  ...
  case 'E':
    ...
    if_it_can_do(eject) rc = scsi_start_unit(tgt, SCSI_CMD_SS_EJECT, ior);
    break;
  ...
  case 'G':
    ...
    case 'P':
      ...
      if_it_can_do(current_position) {
        rc = scsi_read_subchannel(tgt, cmd[13] == 'M', SCSI_CMD_RS_FMT_CURPOS,
                                  0, ior);
        if (rc == SCSI_RET_SUCCESS) {
          ...
          if (cmd[13] == 'M')
            ...
            sprintf(buf, "ABS Position %d %d",
                    (integer_t)(st->subQ.absolute_address.lba.lba1 << 24) +
                        (st->subQ.absolute_address.lba.lba2 << 16) +
                        (st->subQ.absolute_address.lba.lba3 << 8) +
                        st->subQ.absolute_address.lba.lba4,
                    (integer_t)(st->subQ.relative_address.lba.lba1 << 24) +
    ...
    case 'T':
      ...
      if_it_can_do(read_toc) {
        ...
        if (rc == SCSI_RET_SUCCESS) {
          ...
          sprintf(buf, "toc header: %d %d %d", (toc->len1 << 8) + toc->len2,
    ...
    case 'S':
      ...
      if_it_can_do(get_status) {
        rc = scsi_read_subchannel(tgt, TRUE, SCSI_CMD_RS_FMT_CURPOS, 0, ior);
        if (rc == SCSI_RET_SUCCESS) {
          cdrom_chan_curpos_t *st;
          st = (cdrom_chan_curpos_t *)tgt->cmd_ptr;
  ...
  case 'P':
    ...
    case 'A':
      ...
      if_it_can_do(play_msf) {
        ...
        sscanf(&cmd[7], "%d %d %d %d %d %d", &sm, &ss, &sf, &em, &es, &ef);

        rc = scsi_play_audio_msf(tgt, sm, ss, sf, em, es, ef, ior);
      }
      break;
    ...
    case 'T':
      ...
      if_it_can_do(play_ti) {
        ...
        rc = scsi_play_audio_track_index(tgt, st, si, et, ei, ior);
      }
      break;
    }
    break;
  ...
  case 'R':
    /* "Resume" */
    if_it_can_do(pause_resume) rc = scsi_pause_resume(tgt, FALSE, ior);
    break;
  ...
  case 'S':
    ...
    case 'o':
      /* "Stop" */
      if_it_can_do(pause_resume) rc = scsi_pause_resume(tgt, TRUE, ior);
      break;
    ...
    case 't':
      ...
      if_it_can_do(volume_control) {
        ...
        rc = scsi_mode_sense(tgt, SCSI_CD_AUDIO_PAGE, sizeof(au), ior);
        if (rc == SCSI_RET_SUCCESS) {
          ...
          rc = scsi2_mode_select(tgt, FALSE, &au, sizeof(au), ior);
        }
      }
      break;
    }
        ...
        for (t = 0; m > 0; t++, m -= sizeof(struct cdrom_toc_desc)) {
          ...
          if (cmd[4] == 'M')
            sprintf(buf, "%d %d %d %d %d %d\n", toc->descs[t].control,
                    toc->descs[t].adr, toc->descs[t].trackno,
...
private void
/** TODO: Document. */
decode_status(char *buf, unsigned char audio_status) {
  switch (audio_status) {
  case SCSI_CDST_INVALID:
    sprintf(buf, st_invalid);
    break;
  case SCSI_CDST_PLAYING:
    sprintf(buf, st_playing);
    break;
  case SCSI_CDST_PAUSED:
    sprintf(buf, st_paused);
    break;
  case SCSI_CDST_COMPLETED:
    sprintf(buf, st_complete);
    break;
  case SCSI_CDST_ERROR:
    sprintf(buf, st_error);
    break;
  case SCSI_CDST_NO_STATUS:
    sprintf(buf, st_nothing);
    break;
  }
}
...
private void
/** TODO: Document. */
decode_status_1(char *buf, unsigned char audio_status) {
  switch (audio_status) {
  case 0:
    sprintf(buf, st_playing);
    break;
...
private void
/** TODO: Document. */
curse_the_vendor(red_list_t *list, boolean_t not_really) {
  if (not_really)
    return;
  ...
  printf("Vendor: %s Product: %s.. Revision: %s..\n", list->vendor,
         list->product, list->rev);

#define check(x, y, z)                                                         \
  if (list->x)                                                                 \
    printf("Command code x%x %s not supported\n", y, z);
  ...
  check(volume_control, SCSI_CMD_MODE_SELECT, "MODE_SELECT, AUDIO page(0xe)");
 ...
 */
/** TODO: Document. */
cd_strategy(ior) register io_req_t ior;
{
  return rz_simpleq_strategy(ior, cd_start);
}

/** TODO: Document. */
void cd_start(tgt, done) target_info_t *tgt;
boolean_t done;
{
  io_req_t ior;
...
private scsi_ret_t
/** TODO: Document. */
op_not_supported(target_info_t *tgt, char *cmd, io_req_t ior) {
  /*
...
typedef struct {
  unsigned char xxx;
  BITFIELD_2(unsigned char, is_msf : 1, xxx1 : 7);
  unsigned char data_len1;
  unsigned char data_len0;
  unsigned char audio_status;
  BITFIELD_2(unsigned char, control : 4, xxx2 : 4);
  cdrom_addr_t address;
  BITFIELD_2(unsigned char, chan0_select : 4, xxx3 : 4);
  unsigned char chan0_volume;
  BITFIELD_2(unsigned char, chan1_select : 4, xxx4 : 4);
  unsigned char chan1_volume;
  BITFIELD_2(unsigned char, chan2_select : 4, xxx5 : 4);
  unsigned char chan2_volume;
  BITFIELD_2(unsigned char, chan3_select : 4, xxx6 : 4);
...
typedef struct {
  unsigned char xxx0;
  BITFIELD_2(unsigned char, fmt : 1, xxx1 : 7);
  unsigned char xxx[8];
  BITFIELD_2(unsigned char, chan0_select : 4, xxx3 : 4);
  unsigned char chan0_volume;
  BITFIELD_2(unsigned char, chan1_select : 4, xxx4 : 4);
...
private scsi_ret_t
/** TODO: Document. */
rrd42_set_volume(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
...
typedef struct {
  ...
  struct {
    BITFIELD_2(unsigned char, adr : 4, ctrl : 4);
...
typedef struct {
  ...
  BITFIELD_2(unsigned char, ctrl : 4, xxx1 : 4);
...
private scsi_ret_t
/** TODO: Document. */
nec_eject(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
...
private scsi_ret_t
/** TODO: Document. */
nec_subchannel(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
  nec_subch_data_t *st;
  char *buf = tgt->dev_info.cdrom.result;
  scsi_ret_t rc;
...
private scsi_ret_t
/** TODO: Document. */
nec_read_toc(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
  nec_toc_data_t *t;
  char *buf = tgt->dev_info.cdrom.result;
  scsi_ret_t rc;
  ...
  if (cmd[0] == 'G') {
    sprintf(buf, "toc header: %d %d %d",
            sizeof(*t) + sizeof(t->track_info) * (last - first - 1), first,
            last);
    goto out;
  }
...
private scsi_ret_t
/** TODO: Document. */
nec_play(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
  int sm, ss, sf, em, es, ef;
  int st, si, et, ei;
  scsi_ret_t rc;
...
private scsi_ret_t
/** TODO: Document. */
nec_pause_resume(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
...
private scsi_ret_t
/** TODO: Document. */
toshiba_eject(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
...
private scsi_ret_t
/** TODO: Document. */
toshiba_subchannel(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
...
private scsi_ret_t
/** TODO: Document. */
toshiba_read_toc(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
  ...
  if (cmd[0] == 'G') {
    sprintf(buf, "toc header: %d %d %d",
            sizeof(*th) + sizeof(*t) * (last - first + 1), first, last);
    goto out;
  }
  ...
  while (i <= last) {
    ...
    buf += strlen(buf);
    if (cmd[4] == 'M')
      sprintf(buf, "0 0 %d %d %d %d\n", i, bcd_to_decimal(t->address[0]),
              bcd_to_decimal(t->address[1]), bcd_to_decimal(t->address[2]));
    else
      /* THIS IS WRONG */
      sprintf(buf, "0 0 %d %d\n", i,
...
private scsi_ret_t
/** TODO: Document. */
toshiba_play(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
  int sm, ss, sf, em, es, ef;
  int st, si, et, ei;
  scsi_ret_t rc;
...
private scsi_ret_t
/** TODO: Document. */
toshiba_pause_resume(target_info_t *tgt, char *cmd, io_req_t ior) {
  scsi_command_group_2 c;
...
private red_list_t cdrom_exceptions[] = {
    ...
    {"DEC     ", "RRD42", "", 0, 0, 0, 0, 0, 0, 0, 0, rrd42_set_volume},

    /* We have seen a "CD-ROM DRIVE:84 " "1.0 " */
    {"NEC     ", "CD-ROM DRIVE:84", "", op_not_supported, nec_eject,
     nec_subchannel, nec_read_toc, nec_subchannel, nec_play, nec_play,
     nec_pause_resume, op_not_supported},

    /* We have seen a "CD-ROM DRIVE:XM " "3232" */
    {"TOSHIBA ", "CD-ROM DRIVE:XM", "32", op_not_supported, toshiba_eject,
     toshiba_subchannel, toshiba_read_toc, toshiba_subchannel, toshiba_play,
     toshiba_play, toshiba_pause_resume, op_not_supported},

    {"TOSHIBA ", "CD-ROM DRIVE:XM", "33", op_not_supported, toshiba_eject,
     toshiba_subchannel, toshiba_read_toc, toshiba_subchannel, toshiba_play,
     toshiba_play, toshiba_pause_resume, op_not_supported},
...
private void
/** TODO: Document. */
check_red_list(target_info_t *tgt, scsi2_inquiry_data_t *inq)
