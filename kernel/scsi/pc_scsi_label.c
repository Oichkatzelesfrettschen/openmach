...
/** TODO: Document. */
scsi_rw_abs(dev, data, rw, sec, count) dev_t dev;
{
  io_req_t ior;
  io_return_t error;
...
#include "vm/vm_kern.h"
    ...
    *status = 0;
    while (count > 0) {
      ...
      if (scsi_rw_abs(dev, scsi_verify_buf, IO_READ, sec, xcount) !=
          D_SUCCESS) {
        *status = BAD_BLK;
        break;
      } else {
...
unsigned int status_count;
{
  ...
  case V_WRABS:
    ...
    if (scsi_rw_abs(dev, status, IO_WRITE, scsi_abs_sec, DEV_BSIZE) !=
        D_SUCCESS)
      return (D_INVALID_OPERATION);
    break;
  default:
