...
int scsi_tape_timeout =
    5 * 60; /* secs, tk50 is slow when positioning far apart */
...
io_req_t req;
{
  ...
  if (scsi_debug) {
    ...
    printf("[modsns(%x): x%x x%x x%x x%x x%x]", ret, p[0], p[1], p[2], p[3],
           p[4]);
  }
...
again:
  ...
  if (!ior->io_error && (ret == SCSI_RET_SUCCESS)) {
    ...
    tgt->block_size = (lim->minlen_msb << 8) | lim->minlen_lsb;

    maxl = (lim->maxlen_msb << 16) | (lim->maxlen_sb << 8) | lim->maxlen_lsb;
    if (maxl == 0)
      maxl = (unsigned)-1;
...
target_info_t *tgt;
{
  ...
  if (tgt->ior)
    printf("TAPE: Close with pending requests ?? \n");
...
register io_req_t ior;
{
  ...
  if (((ior->io_op & IO_READ) == 0) && tgt->dev_info.tape.read_only) {
...
static void
    /** TODO: Document. */
    do_residue(ior, sns, bsize) io_req_t ior;
...
int bsize;
{
  ...
  residue = sns->u.xtended.info0 << 24 | sns->u.xtended.info1 << 16 |
...
/** TODO: Document. */
void sctape_start(tgt, done) target_info_t *tgt;
boolean_t done;
{
...
natural_t *status_count;
{
  ...
  case TAPE_STATUS: {
    ...
    ts->flags = (tgt->flags & TGT_REWIND_ON_CLOSE) ? TAPE_FLG_REWIND : 0;
...
natural_t status_count;
{
  ...
  retry:
    ...
    case MTOFFL: /* rewind and put the drive offline */
      ...
      iowait(ior);
      if ((short)(mt->mt_operation) == MTREW)
        break;
      ior->io_op = 0;
      ior->io_next = 0;
      ior->io_error = 0;
