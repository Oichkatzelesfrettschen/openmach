...
io_req_t req;
{
  ...
  if (ret == SCSI_RET_SUCCESS) {
    ...
    disk_size =
        (cap->lba1 << 24) | (cap->lba2 << 16) | (cap->lba3 << 8) | cap->lba4;
    if (scsi_debug)
      printf("rz%d holds %d blocks\n", tgt->unit_no, disk_size);
    val = (cap->blen1 << 24) | (cap->blen2 << 16) | (cap->blen3 << 8) |
          cap->blen4;
    if (dev_bsize == 0)
      dev_bsize = val;
    else if (val != dev_bsize)
      panic("read capacity bad");
  ...
  if (dev_bsize == 0) {
    ...
    if (ret == SCSI_RET_SUCCESS) {
      ...
      dev_bsize = m->bdesc[0].blen_msb << 16 | m->bdesc[0].blen << 8 |
                  m->bdesc[0].blen_lsb;
    }
  }
  ...
  if (ret == SCSI_RET_SUCCESS) {
    ...
    secs_per_cyl =
        (cap->lba1 << 24) | (cap->lba2 << 16) | (cap->lba3 << 8) | cap->lba4;
    secs_per_cyl += 1;
    sector_size = (cap->blen1 << 24) | (cap->blen2 << 16) | (cap->blen3 << 8) |
                  cap->blen4;
  } else {
  ...
  fudge_bsd_label(label, DTYPE_SCSI, disk_size /* /(32*64)*/, 64, 32,
                  sector_size, 8);

  numpart = get_only_partition(&scsi_info, (*scsi_read_fun),
                               tgt->dev_info.disk.scsi_array, MAX_SCSI_PARTS,
                               disk_size, drive_name);
 ...
 */
/** TODO: Document. */
void scdisk_start(tgt, done) target_info_t *tgt;
boolean_t done;
{
  ...
  if (done) {
    ...
    if ((tgt->done == SCSI_RET_RETRY) && ((ior->io_op & IO_INTERNAL) == 0)) {
      delay(1000000); /*XXX*/
      goto start;
    } else
      ...
      if ((tgt->done == (SCSI_RET_ABORTED | SCSI_RET_RETRY)) &&
        ...
        if (tgt->cur_cmd == SCSI_CMD_REQUEST_SENSE) {
          ...
          if (sns->addr_valid)
            blockno = sns->u.xtended.info0 << 24 | sns->u.xtended.info1 << 16 |
...
/** TODO: Document. */
void scdisk_start_rw(tgt, ior) target_info_t *tgt;
register io_req_t ior;
{
  ...
  long_form =
      (tgt->flags & TGT_BIG) || (ior->io_count > (256 * tgt->block_size));
...
natural_t status_count;
{
  ...
  case SCSI_FORMAT_UNIT: { /*_IOW(p, 4, struct format_params) */
    ...
    err = scsi_format_unit(tgt, mode, fp->fp_pattern, fp->fp_interleave, ior);
    scsi_watchdog_period = old_timeout;
    /* Make sure we re-read all info afresh */
    tgt->flags =
        TGT_ALIVE | (tgt->flags & (TGT_REMOVABLE_MEDIA | TGT_FULLY_PROBED));
  } break;
  ...
  case SCSI_MODE_SELECT: { /*_IOW(p, 8, struct mode_sel_sns_params) */
    ...
    err = scdisk_mode_select(tgt, DEV_BSIZE, ior, ior->io_data, ms->msp_length,
                             ms->msp_setps);
  } break;
...
/** TODO: Document. */
void scdisk_checksum(bno, addr, size) long bno;
...
/** TODO: Document. */
void scdisk_bcheck(bno, addr, size) long bno;
register unsigned int *addr;
{
  register int i = size / sizeof(int);
  register unsigned int sum = -1;
  ...
  if (scdisk_checksums[SUMHASH(bno)].blockno != bno) {
    if (scsi_debug)
      printf("No checksum for block x%x\n", bno);
    return;
  }
  ...
  if (scdisk_checksums[SUMHASH(bno)].sum != sum) {
    printf("Bad checksum (x%x != x%x), bno x%x size x%x at x%x\n", sum,
           scdisk_checksums[bno & (SUMSIZE - 1)].sum, bno, size, start);
    gimmeabreak();
    scdisk_checksums[SUMHASH(bno)].sum = sum;
  }
}
...
void test_read(max) {
  ...
  for (i = test_read_skew_min; i < max; i++) {
    ...
    printf("Skew %3d size %d count %d time %3d sec %d us\n", i, test_read_size,
           test_read_nreads, stop.tv_sec - start.tv_sec, usecs);
  }
  test_read_skew = ssk;
}
