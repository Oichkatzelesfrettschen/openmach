...
nw_dev_entry_s nc_failure_entry_table = {
    nc_fail,       nc_fail,      nc_null,        nc_null,      nc_null_poll,
    nc_null_send,  nc_null_rpc,  nc_null_signal, nc_open_fail, nc_accept_fail,
    nc_close_fail, nc_open_fail, nc_open_fail};

nw_dev_entry_s nc_local_entry_table = {
    nc_succeed,    nc_succeed,   nc_null,        nc_null,      nc_null_poll,
    nc_local_send, nc_local_rpc, nc_null_signal, nc_open_fail, nc_accept_fail,
    nc_close_fail, nc_open_fail, nc_open_fail};
...
void nc_fast_sweep() {
  ...
  for (dev = 0; dev < MAX_DEV; dev++) {
    if (devct[dev].status == NW_SUCCESS && devct[dev].fast_req > 0) {
      devct[dev].fast_req = 0;
...
nw_result nc_update(nw_update_type up_type, int *up_info) {
  ...
  if (up_type == NW_HOST_ADDRESS_REGISTER) {
    if (nw_free_address == -1) {
      ...
      ad->ip_next = nw_ip[h];
      nw_ip[h] = slot;
      h = (ad->address.nw_addr_1 % MAX_HASH + ad->address.nw_addr_2) % MAX_HASH;
      ad->nw_next = nw_nw[h];
      nw_nw[h] = slot;
      ad->line = 0;
      rc = NW_SUCCESS;
    ...
    ad = &nw_address[slot];
    while (slot != -1 &&
           (ad->address.nw_addr_1 != n1 || ad->address.nw_addr_2 != n2)) {
      previous_slot = slot;
      slot = ad->nw_next;
      ad = &nw_address[slot];
    }
...
nw_result nc_endpoint_allocate(nw_ep_t epp, nw_protocol protocol,
                               nw_acceptance accept, char *buffer_address,
                               u_int buffer_size) {
  nw_result rc;
  nw_ep ep;
  nw_ecb_t ecb;
...
/** TODO: Document. */
nw_result nc_succeed(int dev) { return NW_SUCCESS; }

/** TODO: Document. */
void nc_null(int dev) {}

/** TODO: Document. */
nw_result nc_fail(int dev) { return NW_FAILURE; }

/** TODO: Document. */
int nc_null_poll(int dev) { return 1000000; }
...
/** TODO: Document. */
nw_result nc_open_fail(nw_ep lep, nw_address_1 a1, nw_address_2 a2, nw_ep rep) {

  return NW_FAILURE;
}

/** TODO: Document. */
nw_result nc_close_fail(nw_ep ep) { return NW_FAILURE; }
