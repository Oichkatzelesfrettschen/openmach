...
int (*console_probe)() = 0, (*console_param)() = 0, (*console_start)() = 0,
    (*console_putc)() = 0, (*console_getc)() = 0, (*console_pollc)() = 0,
    (*console_mctl)() = 0, (*console_softCAR)() = 0;
...
/** TODO: Document. */
cngetc() { return (*console_getc)(cnunit, cnline, TRUE, FALSE); }

/** TODO: Document. */
cnpollc(bool) { (*console_pollc)(cnunit, bool); }

/* Debugger support */
/** TODO: Document. */
cnmaygetc() { return (*console_getc)(cnunit, cnline, FALSE, FALSE); }
...
register int line;
{
  return (SCREEN_ISA_CONSOLE() &&
          ((line == SCREEN_LINE_KEYBOARD) || (line == SCREEN_LINE_POINTER)));
}
#endif
...
cons_find(tube) boolean_t tube;
{
  ...
  if ((console_probe == 0) || ((*console_probe)(0, &d) == 0)) {
...
/** TODO: Document. */
cons_open(dev, flag, ior) int dev;
int flag;
io_req_t ior;
{
 ...
 */
/** TODO: Document. */
cons_start(tp) register struct tty *tp;
{
  spl_t s;
 ...
 */
/** TODO: Document. */
cons_stop(tp, flag) register struct tty *tp;
{
  register spl_t s;
 ...
 */
/** TODO: Document. */
cons_mctl(struct tty *tp, int bits, int how) {
 ...
 */
/** TODO: Document. */
cons_portdeath(dev, port) int dev;
mach_port_t port;
{
  if (dev == user_console)
    dev = console;
...
#if NBM > 0
  if (screen_captures(ttyno) &&
      (screen_get_status(SCREEN_CONS_UNIT(), flavor, data, status_count) ==
       D_SUCCESS))
    return D_SUCCESS;
#endif NBM> 0
...
unsigned int status_count;
{
...
#if NBM > 0
  if (screen_captures(ttyno) &&
      (screen_set_status(SCREEN_CONS_UNIT(), flavor, data, status_count) ==
       D_SUCCESS))
    return D_SUCCESS;
#endif NBM> 0
  ...
  case TTY_STATUS: {
    ...
    if ((tsp->tt_ispeed != tp->t_ispeed) || (tsp->tt_ospeed != tp->t_ospeed) ||
    ...
    } else if (tsp->tt_flags & TF_HUPCLS)
...
/** TODO: Document. */
cons_simple_rint(ttyno, line, c, err) int line;
int c;
{
  /*
...
#if MACH_KDB
    if ((err & CONS_ERR_BREAK) || ((c & 0x7f) == l3break))
      return gimmeabreak();
#endif /* MACH_KDB */
    ttyno = console;
    goto process_it;
 ...
 */
/** TODO: Document. */
cons_simple_tint(ttyno, all_sent) boolean_t all_sent;
{
  register struct tty *tp;
