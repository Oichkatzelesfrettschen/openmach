...
nw_result mk_endpoint_allocate_internal(nw_ep_t epp, nw_protocol protocol,
  ...
  if (ep >= MAX_EP || (pv = hect[ep].pv) != NULL) {
    ...
    rc = nc_endpoint_allocate(epp, protocol, accept, (char *)kernel_addr,
                              buffer_size);
    if (rc == NW_NO_EP && (ep = *epp) != 0) {
      rc =
          (*(devct[NW_DEVICE(ect[ep].conn->peer.rem_addr_1)].entry->close))(ep);
      if (rc == NW_SYNCH) {
...
nw_result mk_endpoint_allocate(nw_ep_t epp, nw_protocol protocol,
  ...
  if (invalid_user_access(current_task()->map, (vm_offset_t)epp,
      ...
      (accept != NW_NO_ACCEPT && accept != NW_APPL_ACCEPT &&
       accept != NW_AUTO_ACCEPT)) {
    rc = NW_INVALID_ARGUMENT;
  } else {
    rc = mk_endpoint_allocate_internal(epp, protocol, accept, buffer_size,
                                       FALSE);
  }
  return rc;
}
...
nw_result mk_endpoint_deallocate_internal(nw_ep ep, task_t task,
  ...
  if (ep >= MAX_EP || (pv = hect[ep].pv) == NULL) {
    ...
    if (pv == NULL) {
      ...
      if (projected_buffer_deallocate(task->map, pv->buf_start, pv->buf_end) !=
          KERN_SUCCESS) {
        rc = NW_INCONSISTENCY;
        ...
        if (owned == NULL) {
          ...
          if (hect[ep].pv == NULL) {
            if (ect[ep].state != NW_UNCONNECTED) {
              rc = (*(devct[NW_DEVICE(ect[ep].conn->peer.rem_addr_1)]
                          .entry->close))(ep);
              if (rc == NW_SYNCH) {
                hect[ep].sig_waiter = current_thread();
                assert_wait(0, TRUE);
  ...
  if (ep >= MAX_EP || (pv = hect[ep].pv) == NULL) {
    ...
    if (pv == NULL) {
      ...
      if ((char *)buffer < pv->buf_start ||
        ...
        buffer =
            (nw_buffer_t)((char *)buffer - pv->buf_start + ect[ep].buf_start);
...
nw_result mk_connection_open_internal(nw_ep local_ep, nw_address_1 rem_addr_1,
  ...
  rc = (*devct[NW_DEVICE(rem_addr_1)].entry->open)(local_ep, rem_addr_1,
                                                   rem_addr_2, remote_ep);
  if (rc == NW_SYNCH) {
...
/** TODO: Document. */
nw_result mk_connection_accept(nw_ep ep, nw_buffer_t msg, nw_ep_t new_epp) {
  nw_result rc;
  nw_pv_t pv;
  ...
  if (ep >= MAX_EP || (pv = hect[ep].pv) == NULL) {
    ...
    if (pv == NULL) {
      rc = NW_PROT_VIOLATION;
               ...
               !msg->buf_used || (char *)msg + msg->buf_length > pv->buf_end) {
      rc = NW_BAD_BUFFER;
...
nw_result mk_connection_close(nw_ep ep) {
  ...
  if (ep >= MAX_EP || (pv = hect[ep].pv) == NULL) {
    ...
    if (pv == NULL) {
      ...
      rc = (*devct[NW_DEVICE(ect[ep].conn->peer.rem_addr_1)].entry->close)(ep);
      if (rc == NW_SYNCH) {
        hect[ep].sig_waiter = current_thread();
        assert_wait(0, TRUE);
        current_thread()->nw_ep_waited = NULL;
...
nw_result mk_multicast_add(nw_ep local_ep, nw_address_1 rem_addr_1,
  ...
  if (local_ep >= MAX_EP || (pv = hect[local_ep].pv) == NULL) {
    ...
    if (pv == NULL) {
      ...
      rc = (*(devct[NW_DEVICE(rem_addr_1)].entry->add))(local_ep, rem_addr_1,
                                                        rem_addr_2, remote_ep);
      if (rc == NW_SYNCH) {
        hect[local_ep].sig_waiter = current_thread();
        assert_wait(0, TRUE);
...
nw_result mk_multicast_drop(nw_ep local_ep, nw_address_1 rem_addr_1,
  ...
  if (local_ep >= MAX_EP || (pv = hect[local_ep].pv) == NULL) {
    ...
    if (pv == NULL) {
      ...
      rc = (*(devct[NW_DEVICE(rem_addr_1)].entry->drop))(local_ep, rem_addr_1,
                                                         rem_addr_2, remote_ep);
      if (rc == NW_SYNCH) {
        hect[local_ep].sig_waiter = current_thread();
        assert_wait(0, TRUE);
...
/** TODO: Document. */
nw_result mk_endpoint_status(nw_ep ep, nw_state_t state, nw_peer_t peer) {
  nw_result rc;
  nw_pv_t pv;
      ...
      if (ecb->state == NW_INEXISTENT ||
        ...
        while (header != NULL) {
          if ((char *)msg < pv->buf_start ||
            ...
            header->buffer =
                (nw_buffer_t)((char *)msg - pv->buf_start + ecb->buf_start);
        ...
        if (header == NULL) {
          ...
          if (ecb->protocol != NW_DATAGRAM ||
            ...
            if (sender == -1) {
              ...
              if (rc == NW_SUCCESS) {
                rc = mk_connection_open_internal(
                    sender, first_header->peer.rem_addr_1,
                    first_header->peer.rem_addr_2, MASTER_LINE_EP);
                if (rc == NW_SUCCESS)
          ...
          if (rc == NW_SUCCESS) {
            ...
            if ((rc == NW_SYNCH || rc == NW_QUEUED) && nw_free_waiter != NULL) {
              w = nw_free_waiter;
        ...
        if (header == NULL) {
          ...
          if (ecb->protocol != NW_DATAGRAM ||
            ...
            if (sender == -1) {
              ...
              nrc = mk_endpoint_allocate_internal(&sender, NW_LINE,
                                                  NW_AUTO_ACCEPT, 0, TRUE);
              if (nrc == NW_SUCCESS) {
                nrc = mk_connection_open_internal(
                    sender, first_header->peer.rem_addr_1,
                    first_header->peer.rem_addr_2, MASTER_LINE_EP);
                if (nrc == NW_SUCCESS)
                  nc_line_update(&first_header->peer, sender);
          ...
          if (nrc == NW_SUCCESS) {
            ...
            if (rc != NULL && rc != NW_BUFFER_ERROR) {
              rc = (nw_buffer_t)((char *)rc - ecb->buf_start + pv->buf_start);
  ...
  if (invalid_user_access(current_task()->map, (vm_offset_t)epp,
    ...
    if (i == nep) {
      if (time_out == 0) {
        ...
        waited = nw_free_waited;
        i = 0;
        while (i < nep && nw_free_waiter != NULL && nw_free_waited != NULL) {
...
/** TODO: Document. */
void mk_return() { thread_syscall_return(current_thread()->wait_result); }
