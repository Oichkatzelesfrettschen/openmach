...
struct se_init_block {
  ...
  BITFIELD_3(unsigned char, recv_ring_pointer_hi, /* Receive Ring ptr, high  */
             reserved0 : 5, recv_ring_len : 3);   /* Length: log2(nbuffers)  */
  unsigned short xmit_ring_pointer_lo;            /* Transmit Ring ptr, low  */
  BITFIELD_3(unsigned char, xmit_ring_pointer_hi, /* Transmit Ring ptr, high */
             reserved1 : 5, xmit_ring_len : 3);   /* Length: log2(nbuffers)  */
};
...
struct se_desc {
  unsigned short addr_low;           /* (H)  Buffer pointer low  */
                                     /** TODO: Document. */
  BITFIELD_2(unsigned char, addr_hi, /* (H)  Buffer pointer high */
             status);                /* (HC) Buffer status	*/
  unsigned short buffer_size;        /* (H)  Buffer length (bytes),*/
                                     /* bits 15..12 must be ones */
  union {
    struct {
      BITFIELD_2(unsigned short, bcnt : 12, /* (C)  Rcvd data size	*/
...
#define LN_MINBUF_CH                                                           \
  100 /* Minimum size of first lance buffer, if                                \
         chaining */

#define LN_MINBUF_NOCH                                                         \
  60 /* Minimum size of a lance buffer, if                                     \
        no chaining and DTCR==1 */

#define LN_MINBUF_NOCH_RAW                                                     \
  64 /* Minimum size of a lance buffer, if                                     \
