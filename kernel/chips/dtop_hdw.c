...
struct dtop_softc {
  ...
  struct dtop_ds {
    int (*handler)(dtop_device_t, dtop_message_t, int, unsigned char);
    dtop_device status;
...
/** TODO: Document. */
set_dtop_address(dtopunit, poll_reg) data_reg_t poll_reg;
{
  int i;

  extern int dtop_probe(), dtop_param(), dtop_start(), dtop_putc(), dtop_getc(),
...
/** TODO: Document. */
dtop_probe(data_reg, ui) data_reg_t data_reg;
struct bus_device *ui;
{
  ...
  dtop->device[DTOP_DEVICE_NO(0x6a)].status.locator.type = DEV_MOUSE;
  dtop->device[DTOP_DEVICE_NO(0x6a)].status.locator.relative = 1;
  ...
  dtop->device[DTOP_DEVICE_NO(0x6a)].status.locator.button_code[2] =
      KEY_MIDDLE_BUTTON;
  dtop->device[DTOP_DEVICE_NO(0x6a)].status.locator.n_coords = 2;
...
static void
    /** TODO: Document. */
    dtop_attach(ui) struct bus_device *ui;
{
  int i;
 ...
 */
/** TODO: Document. */
dtop_pollc(unit, on) boolean_t on;
{
  dtop_softc_t dtop;
 ...
 */
/** TODO: Document. */
dtop_intr(unit, spllevel, recvd) spl_t spllevel;
boolean_t recvd;
{
...
/* Conversely... */
/** TODO: Document. */
dtop_put_packet(dtop, pkt) dtop_softc_t dtop;
dtop_message_t pkt;
{
  register int i, max;
  ...
  while (i < pkt->code.val.len + 2) {
    for (max = 0; max < DTOP_MAX_POLL && !DTOP_TX_AVAIL(dtop->poll); max++)
      ;
    if (max == DTOP_MAX_POLL)
      goto bad;
 ...
 */
/** TODO: Document. */
dtop_getc(unit, line, wait, raw) boolean_t wait;
boolean_t raw;
{
  register int c;
  dtop_softc_t dtop;
  ...
  if (DTOP_RX_AVAIL(dtop->poll)) {
    ...
    if (dtop_get_packet(dtop, &msg) >= 0) {
      ...
      if (ds->handler == dtop_keyboard_handler) {
        ...
        c = dtop_keyboard_handler(&ds->status, &msg, DTOP_EVENT_RECEIVE_PACKET,
                                  -1);
...
dtop_param(tp, line) struct tty *tp;
{
  if (tp->t_ispeed == 0)
    ttymodem(tp, 0);
  else
 ...
 */
/** TODO: Document. */
dtop_mctl(dev, bits, how) int dev;
int bits, how;
{
  return 0;
}
...
/** TODO: Document. */
dtop_kbd_reset(unit) { return lk201_reset(unit); }
