...
/** TODO: Document. */
ims332_write_register(regs, regno, val) unsigned char *regs;
...
#define ims332_read_register(p, r) ((unsigned int *)(p))[(r)]
#define ims332_write_register(p, r, v) ((unsigned int *)(p))[(r)] = (v)
 ...
 */
/** TODO: Document. */
ims332_load_colormap(regs, map) ims332_padded_regmap_t *regs;
color_map_t *map;
{
  register int i;
...
/** TODO: Document. */
ims332_load_colormap_entry(regs, entry, map) ims332_padded_regmap_t *regs;
color_map_t *map;
{
  /* ?? stop VTG */
  ims332_write_register(regs, IMS332_REG_LUT_BASE + (entry & 0xff),
                        (map->blue << 16) | (map->green << 8) | (map->red));
}

/** TODO: Document. */
ims332_init_colormap(regs) ims332_padded_regmap_t *regs;
{
  color_map_t m;
...
#if 1 /*debug*/
/** TODO: Document. */
ims332_print_colormap(regs) ims332_padded_regmap_t *regs;
{
  register int i;
...
ims332_video_off(vstate, up) struct vstate *vstate;
user_info_t *up;
{
...
ims332_video_on(vstate, up) struct vstate *vstate;
user_info_t *up;
{
  register ims332_padded_regmap_t *regs = vstate->regs;
  register unsigned *save, csr;
 ...
 */
/** TODO: Document. */
ims332_pos_cursor(regs, x, y) ims332_padded_regmap_t *regs;
register int x, y;
{
...
/** TODO: Document. */
ims332_cursor_color(regs, color) ims332_padded_regmap_t *regs;
color_map_t *color;
{
  /* Bg is color[0], Fg is color[1] */
  ims332_write_register(regs, IMS332_REG_CURSOR_LUT_0,
                        (color->blue << 16) | (color->green << 8) |
                            (color->red));
  color++;
...
/** TODO: Document. */
ims332_cursor_sprite(regs, cursor) ims332_padded_regmap_t *regs;
unsigned short *cursor;
{
  ...
  for (i = 0; i < 512; i++, cursor++)
    ims332_write_register(regs, IMS332_REG_CURSOR_RAM + i, *cursor);
}
 ...
 */
/** TODO: Document. */
ims332_init(regs, reset, mon) ims332_padded_regmap_t *regs;
unsigned int *reset;
xcfb_monitor_type_t mon;
{
  ...
  frontporch = mon->line_time - (mon->half_sync * 2 + mon->back_porch +
                                 mon->frame_visible_width / 4);

  shortdisplay =
      mon->line_time / 2 - (mon->half_sync * 2 + mon->back_porch + frontporch);
  ...
  ims332_write_register(regs, IMS332_REG_DISPLAY, mon->frame_visible_width / 4);
  ...
  ims332_write_register(regs, IMS332_REG_CSR_A,
                        IMS332_BPP_8 | IMS332_CSR_A_DMA_DISABLE |
