...
io_return_t
/** TODO: Document. */
lk201_translation(int key, char c, int tabcode);

int
/** TODO: Document. */
lk201_input(int unit, unsigned short data);

/*
 * Autoconf (sort-of)
 */
/** TODO: Document. */
lk201_probe(int unit) {
  lk201_softc[unit] = &lk201_softc_data[unit];
  return 1;
}

/** TODO: Document. */
void lk201_attach(int unit, int sl_unit) {
...
static unsigned char lk201_reset_string[] = {
    ...
    LK_CMD_SET_DEFAULTS, LK_CMD_MODE(LK_MODE_RPT_DOWN, 1),
    LK_CMD_MODE(LK_MODE_RPT_DOWN, 2), LK_CMD_MODE(LK_MODE_RPT_DOWN, 3),
    LK_CMD_MODE(LK_MODE_RPT_DOWN, 4), LK_CMD_MODE(LK_MODE_DOWN_UP, 5),
    LK_CMD_MODE(LK_MODE_DOWN_UP, 6), LK_CMD_MODE(LK_MODE_RPT_DOWN, 7),
    LK_CMD_MODE(LK_MODE_RPT_DOWN, 8), LK_CMD_MODE(LK_MODE_RPT_DOWN, 9),
    LK_CMD_MODE(LK_MODE_RPT_DOWN, 10), LK_CMD_MODE(LK_MODE_RPT_DOWN, 11),
    LK_CMD_MODE(LK_MODE_RPT_DOWN, 12), LK_CMD_MODE(LK_MODE_DOWN, 13),
    LK_CMD_MODE(LK_MODE_RPT_DOWN, 14), LK_CMD_ENB_RPT,
    /*	LK_CMD_ENB_KEYCLK, LK_PARAM_VOLUME(4),	*/
    LK_CMD_DIS_KEYCLK, LK_CMD_RESUME, LK_CMD_ENB_BELL, LK_PARAM_VOLUME(4),
    LK_CMD_LEDS_OFF, LK_PARAM_LED_MASK(0xf)};

void
/** TODO: Document. */
lk201_reset(int unit) {
  register int i, sl;
  register spl_t s;
  lk201_softc_t lk;
  ...
  for (i = 0; i < sizeof(lk201_reset_string); i++) {
    (*console_putc)(sl, SCREEN_LINE_KEYBOARD, lk201_reset_string[i]);
    delay(100);
  }
...
lk201_selftest(int unit) {
  ...
  if (messg[0] != 0x01 || messg[1] || messg[2] || messg[3])
    printf("bad [%x %x %x %x]", messg[0], messg[1], messg[2], messg[3]);
  else
    printf("ok )");
...
void
/** TODO: Document. */
lk201_ring_bell(int unit) {
  spl_t s = spltty();
  (*console_putc)(lk201_softc[unit]->sl_unit, SCREEN_LINE_KEYBOARD,
                  LK_CMD_BELL);
  splx(s);
}
...
void
/** TODO: Document. */
lk201_lights(int unit, boolean_t on) {
  unsigned int sl;
  spl_t s;
  ...
  if (leds->led_interval) {           /* leds are on */
    if (leds->led_light_count <= 0) { /* hit this lights */
      ...
      if (leds->led_lights <= 0)
        leds->led_lights = 1; /* sanity */
      if (leds->led_lights > 16)
        leds->led_lights = 16;                    /* sanity */
      leds->led_light_count = leds->led_interval; /* reset */
      s = spltty();
      (*console_putc)(sl, SCREEN_LINE_KEYBOARD, LK_CMD_LEDS_OFF);
      (*console_putc)(sl, SCREEN_LINE_KEYBOARD,
                      LK_PARAM_LED_MASK(leds->led_lights));
...
io_return_t
/** TODO: Document. */
lk201_set_status(int unit, dev_flavor_t flavor, dev_status_t status,
  ...
  case LK201_SEND_CMD: {
    ...
    if ((status_count < (sizeof(*cmd) / sizeof(int))) || ((cnt = cmd->len) > 2))
  ...
  case LK201_LED_PATTERN: {
    ...
    if (ptn != LED_OFF && ptn != LED_COUNT && ptn != LED_ROTATE &&
        ptn != LED_CYLON) {
      return -1;
    } else {
  ...
  case LK201_REMAP_KEY: {
    ...
    if (k->meta)
      mode += 2;
    return lk201_translation(k->in_keyval, k->out_keyval, mode);
  }
  default:
...
/** TODO: Document. */
lk201_rint(int unit, unsigned short data, boolean_t handle_shift,
...
lk201_input(int unit, unsigned short data) {
  ...
  default:
    ...
    if (kbd->kbd_ctrl || kbd->kbd_lock || kbd->kbd_shift) {
      c = ((kbd->kbd_meta) ? lk201_xlate_shifted_meta
                           : lk201_xlate_shifted)[data - LK_MINCODE];
      if (kbd->kbd_ctrl)
        c &= 0x1f;
    } else
      c = ((kbd->kbd_meta) ? lk201_xlate_meta
                           : lk201_xlate_key)[data - LK_MINCODE];
    break;
  }
