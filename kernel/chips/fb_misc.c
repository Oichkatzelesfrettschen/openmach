 ...
 */
/** TODO: Document. */
fb_init_colormap(sc) screen_softc_t sc;
{
 ...
 */
/** TODO: Document. */
fb_small_cursor_to_large(up, cursor) user_info_t *up;
 ...
 */
/** TODO: Document. */
fb_set_status(sc, flavor, status, status_count) screen_softc_t sc;
int flavor;
dev_status_t status;
unsigned int status_count;
{
  ...
  case SCREEN_LOAD_CURSOR_LONG: { /* 3max/3min only */
    ...
    bt431_cursor_sprite(fb->cursor_registers,
                        sc->up->dev_dep_2.pm.cursor_sprite);
    sc->flags &= ~SCREEN_BEING_UPDATED;
 ...
 */
/** TODO: Document. */
fb_soft_reset(sc) screen_softc_t sc;
{
  fb_softc_t *fb = (fb_softc_t *)sc->hw_state;
  user_info_t *up = sc->up;
