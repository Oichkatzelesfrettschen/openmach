...
struct bus_driver fd_driver = {
    fd_probe, fd_slave, fd_attach, fd_go,    fd_std,
    "fd",     fd_info,  "fdc",     fd_minfo, /*BUS_INTR_B4_PROBE*/};
 ...
 */
/** TODO: Document. */
static fd_probe(reg, ctlr) vm_offset_t reg;
struct bus_ctlr *ctlr;
{
  ...
  if (the_fdc_type == fdc_82077aa) {
    ...
    if (temp != FD_VERSION_82077AA)
      printf("{ %s x%x } ", "Accepting non-82077aa version id", temp);
  }

  printf("%s%d: %s chip controller", ctlr->name, ctlr->unit,
         chip_names[fd->fdc_type]);
  if (the_fdc_type == fdc_82077aa)
...
static fd_slave(ui, reg) struct bus_device *ui;
vm_offset_t reg;
{
  int slave = ui->slave;
  fd_softc_t fd;
  ...
  if (!fd_go(fd, slave, sns, 2, 1))
    return 0;
  if (!fd_get_result(fd, sns, 1, FALSE))
    return 0;
...
static void
    /** TODO: Document. */
    fd_attach(ui) struct bus_device *ui;
{
  /* Attach a slave */
}
...
fd_reset(fd) fd_softc_t fd;
{
   ...
   */
  {
    ...
    } while ((nloops < 30) && ((i < 4) || (st[0] != FD_ST0_IC_BAD_CMD)));
...
char *cmd;
{
  ...
  while (cmd_len > 0) {
    ...
    for (i = 0; i < FD_MAX_WAIT; i++)
      if ((regs->fd_msr & (FD_MSR_RQM | FD_MSR_DIO)) == FD_MSR_RQM)
        break;
      else
        delay(10);
    if (i == FD_MAX_WAIT) {
...
fd_state(unit) {
  ...
  if (the_fdc_type == fdc_8272a)
    ...
    printf("sra %x srb %x dor %x tdr %x msr %x dir %x\n", regs->fd_sra,
           regs->fd_srb, regs->fd_dor, regs->fd_tdr, regs->fd_msr,
           regs->fd_dir);
}
...
fd_close(dev) int dev;
{
  ...
  if (the_fdc_type == fdc_82072) {
    ...
    cmd[0] =
        FD_CMD_MOTOR_ON_OFF | ((slave << FD_CMD_MOT_DS_SHIFT) & FD_CMD_MOT_DS);
...
/** TODO: Document. */
fd_start(fd, slave, done) boolean_t done;
fd_softc_t fd;
{
...
/*	setup dma	*/
  ...
  *(unsigned int *)0xbc040070 = (((unsigned int)kvtophys(ior->io_data)) >> 2)
                                << 5;
...
/** TODO: Document. */
fd_set_status(dev, flavor, status, status_count) int dev;
int flavor;
dev_status_t status;
...
/** TODO: Document. */
fd_get_status(dev, flavor, status, status_count) int dev;
int flavor;
dev_status_t status;
unsigned int status_count;
{
