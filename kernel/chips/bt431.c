...
void
    /** TODO: Document. */
    bt431_select_reg(regs, regno) bt431_padded_regmap_t *regs;
{
...
void
    /** TODO: Document. */
    bt431_write_reg(regs, regno, val) bt431_padded_regmap_t *regs;
{
...
#define bt431_read_reg_autoi(regs, regno) get_value(((regs)->addr_reg))
...
#define bt431_read_cmap_autoi(regs, regno) ((regs)->addr_cmap)

/*
 * Cursor ops
 */
/** TODO: Document. */
bt431_cursor_on(regs) bt431_padded_regmap_t *regs;
{
...
/** TODO: Document. */
bt431_cursor_off(regs) bt431_padded_regmap_t *regs;
{
  bt431_write_reg(regs, BT431_REG_CMD, BT431_CMD_4_1_MUX);
}

/** TODO: Document. */
bt431_pos_cursor(regs, x, y) bt431_padded_regmap_t *regs;
register int x, y;
{
#define lo(v) ((v) & 0xff)
...
/** TODO: Document. */
bt431_cursor_sprite(regs, cursor) bt431_padded_regmap_t *regs;
...
#if 1
/** TODO: Document. */
bt431_print_cursor(regs) bt431_padded_regmap_t *regs;
{
  ...
  for (i = 0; i < 512; i += 16)
    printf("%x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x\n", curs[i],
           curs[i + 1], curs[i + 2], curs[i + 3], curs[i + 4], curs[i + 5],
           curs[i + 6], curs[i + 7], curs[i + 8], curs[i + 9], curs[i + 10],
           curs[i + 11], curs[i + 12], curs[i + 13], curs[i + 14],
           curs[i + 15]);
}
...
unsigned /*char*/ short bt431_default_cursor[64 * 8] = {
    0xffff, 0, 0, 0, 0, 0, 0, 0, 0xffff, 0, 0, 0, 0, 0, 0, 0,
    0xffff, 0, 0, 0, 0, 0, 0, 0, 0xffff, 0, 0, 0, 0, 0, 0, 0,
    0xffff, 0, 0, 0, 0, 0, 0, 0, 0xffff, 0, 0, 0, 0, 0, 0, 0,
    0xffff, 0, 0, 0, 0, 0, 0, 0, 0xffff, 0, 0, 0, 0, 0, 0, 0,
    0xffff, 0, 0, 0, 0, 0, 0, 0, 0xffff, 0, 0, 0, 0, 0, 0, 0,
...
/** TODO: Document. */
bt431_init(regs) bt431_padded_regmap_t *regs;
{
  register int i;
