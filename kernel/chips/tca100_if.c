...
nw_dev_entry_s tca100_entry_table = {
    ...
    tca100_poll,       tca100_send,   tca100_rpc,        spans_input,
    spans_open,        spans_accept,  spans_close,       spans_add,
    spans_drop};
...
nw_result tca100_send(nw_ep ep, nw_tx_header_t header, nw_options options) {
  ...
  tcb = &tct[ep];
  if ((options == NW_URGENT &&
       header->msg_length > MTU_URGENT[ecb->protocol]) ||
      header->msg_length > MTU[ecb->protocol]) {
    rc = NW_BAD_LENGTH;
  } else if (tcb->tx_queued_count != 0 ||
             (ecb->protocol != NW_RAW && long_tx_count[dev] >= MAX_LONG_TX &&
...
nw_result tx_slot_free(int dev, nw_control_t control) {
  ...
  do {
    ...
    if (tx_header == NULL) {
      ...
      while (tx_header->next != NULL && tx_header->next->msg_length == 0) {
        tx_header = tx_header->next;
      }
...
nw_result rx_slot_free(int dev, nw_control_t control) {
  ...
  if (control == NULL) {
    ...
    while ((rx_header = delayed_rx_first[dev]) != NULL &&
      ...
      nc_buffer_deallocate(rx_header->buffer->peer.local_ep, rx_header->buffer);
...
int tca100_poll(int dev) {
  ...
  u_int atm_header, sar_header, predicted_sar_trailer, cs_header, end_count,
      cs_pad, rx_cell_total, reply, block_length, initial_offset;
  ...
  while ((rx_cell_count = *rx_counter & RX_COUNTER_MASK) != 0) {
    ...
    while (rx_cell_count-- > 0) {
      ...
      if ((predicted_sar_header & SEG_TYPE) == COM) {
        ...
        predicted_sar_header =
            (predicted_sar_header + SEQ_INC) & (SEQ_NO | MID);
      ...
      cs_header_check:
        block_length = msg_count =
            (((cs_header >> 8) & 0xff00) | (cs_header >> 24));
        ...
        if (reply == TCA_ACK) {
          if (rx_seqno == tx_seqno && rx_count == tx_count) {
            if (rx_count == 0) {
              ...
              if (tca100_window_send(dev, tx_ecb, tx_tcb, FALSE) ==
                  NW_SUCCESS) {
          ...
          if (rx_seqno == tx_seqno && rx_count == 0xffff &&
              ((int)tx_ecb->tx_initial->msg_length - (int)tx_tcb->tx_synch) <=
          ...
          if (rx_seqno == tx_seqno &&
            ...
            if (++control->retry < MAX_RETRY) {
              ...
              if (tca100_window_send(dev, tx_ecb, tx_tcb, TRUE) == NW_SUCCESS) {
                nc_deliver_result(control->ep, NW_SEND, NW_SUCCESS);
                tx_tcb->reply = NW_SUCCESS;
                tx_slot_free(dev, control);
              } else {
