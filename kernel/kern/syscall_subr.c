...
#include <mach/policy.h>
         ...
         *	lock or condition:  If FALSE is returned, the thread is
         *justified in becoming a resource hog by continuing to spin because
         *there's nothing else useful that the processor could do.  If TRUE is
...
/**
 * Yield the processor to another ready thread if possible.
 */
boolean_t swtch() {
  register processor_t myprocessor;
...
/**
 * Yield the processor and depress priority for a short time.
 *
 * @param pri Ignored in this implementation.
 */
...
/**
 * Perform a context switch with an optional thread hint.
 *
 * @param thread_name Target thread port or 0.
 * @param option Scheduling option.
 * @param option_time Option argument for timeout or depression.
 */
...
mach_msg_timeout_t option_time;
{
   ...
   */
  if ((thread_name != 0) &&
      (ipc_port_translate_send(cur_thread->task->itk_space, thread_name,
                               &port) == KERN_SUCCESS)) {
...
#if NCPUS > 1
  myprocessor = current_processor();
  if (myprocessor->processor_set->runq.count > 0 || myprocessor->runq.count > 0)
#endif NCPUS> 1
  {
 ...
 */
void thread_depress_priority(thread, depress_time) register thread_t thread;
 ...
 */
void thread_depress_timeout(thread) register thread_t thread;
{
  spl_t s;
